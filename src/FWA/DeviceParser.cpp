...
UnitDescriptorSizes
parseUnitIdentifierDescriptorSizes(const std::vector<uint8_t> &data) {
  UnitDescriptorSizes sizes;
  if (data.size() < 6) {
    spdlog::error("parseUnitIdentifierDescriptorSizes: Data too short ({}) for "
                  "essential fields.",
                  data.size());
    return sizes;
  }
  ...
  sizes.valid = true;
  spdlog::debug("Parsed Unit Descriptor Sizes: GenID=0x{:02x}, ListIDSize={}, "
                "ObjIDSize={}, EntryPosSize={}",
                sizes.generationId, sizes.sizeOfListId, sizes.sizeOfObjectId,
                sizes.sizeOfEntryPos);
  return sizes;
}
...
std::optional<ParsedMusicIdentifierData>
parseMusicSubunitIdentifier(const std::vector<uint8_t> &subunitData,
                            size_t unitSizeOfListId, size_t unitSizeOfObjectId,
                            size_t unitSizeOfEntryPos) {
  // <--- START Logging inside parseMusicSubunitIdentifier --->
  spdlog::debug("Entering parseMusicSubunitIdentifier (Data size: {})",
                subunitData.size());
  size_t previewSize = std::min(subunitData.size(), (size_t)20);
  spdlog::debug("  Subunit Data Preview: {}",
                Helpers::formatHexBytes(std::vector<uint8_t>(
                    subunitData.begin(), subunitData.begin() + previewSize)));

  if (subunitData.size() < 10) {
    spdlog::error("  parseMusicSubunitIdentifier: Data too short ({}) for "
                  "header. Returning nullopt.",
                  subunitData.size());
    return std::nullopt;
  }
  // ...existing code...
  ...
  if (subunitData.size() < subunitDepInfoLenOffset + 2) {
    spdlog::error("  parseMusicSubunitIdentifier: Data too short ({}) for "
                  "subunit_type_dependent_information_length at offset {}. "
                  "Returning nullopt.",
                  subunitData.size(), subunitDepInfoLenOffset);
    return std::nullopt;
  }
  uint16_t subunitDepInfoLen = DescriptorUtils::readBytes(
      subunitData.data() + subunitDepInfoLenOffset, 2);
  size_t subunitDepInfoOffset = subunitDepInfoLenOffset + 2;
  spdlog::debug("  Parsed subunitDepInfoLen = {} at offset {}",
                subunitDepInfoLen, subunitDepInfoLenOffset);

  if (subunitData.size() < subunitDepInfoOffset + subunitDepInfoLen) {
    spdlog::error("  parseMusicSubunitIdentifier: Data too short ({}) for "
                  "claimed subunit_type_dependent_information (len {}) at "
                  "offset {}. Returning nullopt.",
                  subunitData.size(), subunitDepInfoLen, subunitDepInfoOffset);
    return std::nullopt;
  }
  ...
  spdlog::debug(
      "  Parsing Music Specific Dependent Info (Available length: {})",
      musicInfoAvailableLen);

  if (musicInfoAvailableLen < 5) {
    spdlog::error("  parseMusicSubunitIdentifier: Music Subunit Dependent Info "
                  "too short ({}) for its header. Returning nullopt.",
                  musicInfoAvailableLen);
    return std::nullopt;
  }
  ...
  uint16_t musicSpecificInfoLen =
      DescriptorUtils::readBytes(musicInfoPtr + 4, 2);
  size_t musicSpecificInfoOffset = 6;

  spdlog::debug("  Music Subunit Version: 0x{:02x}, Specific Info Length: {}",
                caps.musicSubunitVersion, musicSpecificInfoLen);

  if (musicInfoAvailableLen < musicSpecificInfoOffset + musicSpecificInfoLen) {
    spdlog::error(
        "  parseMusicSubunitIdentifier: Music Subunit Dependent Info too short "
        "({}) for claimed specific_information length ({}). Returning nullopt.",
        musicInfoAvailableLen, musicSpecificInfoLen);
    return std::nullopt;
  }
  ...
  spdlog::debug(
      "  Parsing music_subunit_specific_information (Available length: {})",
      specificAvailableLen);

  if (specificAvailableLen < 1) {
    spdlog::warn("  parseMusicSubunitIdentifier: Music Subunit Specific Info "
                 "area is empty. Returning partially filled caps.");
    return std::make_tuple(
        caps,
        std::vector<uint8_t>(musicInfoPtr, musicInfoPtr + subunitDepInfoLen),
        std::vector<uint8_t>());
  }
  ...
  spdlog::debug(
      "    -> Flags: Gen={}, Aud={}, MIDI={}, SMPTE={}, Samp={}, Sync={}",
      caps.hasGeneralCapability, caps.hasAudioCapability,
      caps.hasMidiCapability, caps.hasSmpteTimeCodeCapability,
      caps.hasSampleCountCapability, caps.hasAudioSyncCapability);

  // --- Parse General Capability ---
  if (caps.hasGeneralCapability) {
    spdlog::debug("    Parsing General Capability...");
    if (specificAvailableLen < currentSpecificOffset + 1)
    ...
    if (specificAvailableLen < currentSpecificOffset + genCapTotalBlockSize ||
        genCapLenByte < 6) {
      spdlog::error(
          "      General Capability block size invalid or data too short.");
      goto parse_error_exit;
    }
    ...
    spdlog::debug(
        "      -> TxFlags=0x{:02x}, RxFlags=0x{:02x}, Latency=0x{:08x}",
        caps.transmitCapabilityFlags.value(),
        caps.receiveCapabilityFlags.value(), caps.latencyCapability.value());
  ...
  if (caps.hasAudioCapability) {
    ...
    if (specificAvailableLen < currentSpecificOffset + audioCapTotalBlockSize ||
        audioCapLenByte < 5) {
      spdlog::error(
          "      Audio Capability block size invalid or data too short.");
      goto parse_error_exit;
    }
    ...
    if (audioCapLenByte < minRequiredDataLen) {
      spdlog::error("      Audio Capability data length ({}) insufficient for "
                    "{} formats.",
                    audioCapLenByte, numFormats);
      goto parse_error_exit;
    }

    caps.maxAudioInputChannels = DescriptorUtils::readBytes(audioCapPtr + 1, 2);
    caps.maxAudioOutputChannels =
        DescriptorUtils::readBytes(audioCapPtr + 3, 2);
    spdlog::debug("      -> MaxInCh={}, MaxOutCh={}, NumFormats={}",
                  caps.maxAudioInputChannels.value_or(0),
                  caps.maxAudioOutputChannels.value_or(0), numFormats);
    ...
    for (uint8_t i = 0; i < numFormats; ++i) {
      ...
      spdlog::debug(
          "      -> Format {}: FDF=0x{:02x}, Label=0x{:02x} (Parsing TBD)", i,
          audioCapPtr[formatOffset], audioCapPtr[formatOffset + 1]);
  ...
  if (caps.hasMidiCapability) {
    ...
    if (specificAvailableLen < currentSpecificOffset + midiCapTotalBlockSize ||
        midiCapLenByte < 6)
      goto parse_error_exit;
    ...
    spdlog::debug(
        "      -> MaxIn={}, MaxOut={}, MIDI Ver={}.{}, Adapt Ver=0x{:02x}",
  ...
  if (caps.hasSmpteTimeCodeCapability) {
    ...
    if (specificAvailableLen < currentSpecificOffset + smpteCapTotalBlockSize ||
        smpteCapLenByte < 1)
      goto parse_error_exit;
    caps.smpteTimeCodeCapabilityFlags = specificPtr[currentSpecificOffset + 1];
    spdlog::debug("      -> Flags=0x{:02x}",
                  caps.smpteTimeCodeCapabilityFlags.value());
  ...
  if (caps.hasAudioSyncCapability) {
    ...
    if (specificAvailableLen < currentSpecificOffset + syncCapTotalBlockSize ||
        syncCapLenByte < 1)
      goto parse_error_exit;
    caps.audioSyncCapabilityFlags = specificPtr[currentSpecificOffset + 1];
    spdlog::debug("      -> Flags=0x{:02x}",
                  caps.audioSyncCapabilityFlags.value());
  ...
  return std::make_tuple(
      caps,
      std::vector<uint8_t>(musicInfoPtr, musicInfoPtr + subunitDepInfoLen),
      std::vector<uint8_t>(specificPtr, specificPtr + musicSpecificInfoLen));

parse_error_exit:
  spdlog::error("parseMusicSubunitIdentifier: Error during parsing specific "
                "capability block at offset {}. Returning nullopt.",
                currentSpecificOffset);
  return std::nullopt;
...
DeviceParser::DeviceParser(AudioDevice *device)
    : device_(device), commandInterface_(nullptr), info_(device->info_),
      descriptorMechanismSupported_{false} {
  if (!device_) {
  ...
  if (auto iface = device_->getCommandInterface()) {
    ...
    if (!commandInterface_) {
      spdlog::info(
          "DeviceParser: Failed to get CommandInterface from AudioDevice.");
    }
  } else {
...
static std::expected<std::vector<uint8_t>, IOKitError>
readDescriptorNonStandard(
    CommandInterface *commandInterface, uint8_t targetAddr,
    const std::vector<uint8_t> &specifier,
    uint16_t offset, // Initial offset requested (usually 0 for full read)
    uint16_t
        length) // Requested length (0 means read all based on internal length)
{
  if (!commandInterface) {
    return std::unexpected(IOKitError(kIOReturnNotReady));
  }

  spdlog::warn("readDescriptorNonStandard: Attempting non-standard READ "
               "(CONTROL ctype) for Addr=0x{:02x}, Specifier={}",
               targetAddr, Helpers::formatHexBytes(specifier));

  std::vector<uint8_t> accumulatedData;
  uint16_t currentDeviceOffset =
      offset; // Offset *within the descriptor* to request
  ...
  const uint16_t MAX_CHUNK_REQUEST =
      128; // How much data to ask for per request
  ...
  do {
    ...
    if (lengthDetermined && length == 0) { // Read all mode, length known
      if (totalExpectedDataLength >= accumulatedData.size()) {
        uint16_t remaining = totalExpectedDataLength -
                             static_cast<uint16_t>(accumulatedData.size());
      ...
      uint16_t remaining =
          length - static_cast<uint16_t>(accumulatedData.size());
      readChunkSize = std::min(remaining, MAX_CHUNK_REQUEST);
    }
    ...
    if (readChunkSize == 0 && (lengthDetermined || length > 0)) {
      spdlog::debug(
          "readDescriptorNonStandard: Requested read length achieved.");
    ...
    spdlog::trace("Sending Non-Standard READ (Attempt {}): Offset={}, "
                  "ChunkSize={}, Cmd={}",
                  attempts, currentDeviceOffset, readChunkSize,
                  Helpers::formatHexBytes(cmd));
    auto result = commandInterface->sendCommand(cmd);

    // --- Check Response ---
    if (!result) {
      spdlog::error("readDescriptorNonStandard: sendCommand failed: 0x{:x}",
    ...
    spdlog::trace("Non-Standard READ response status: 0x{:02x}",
                  responseStatus);

    if (responseStatus != kAVCAcceptedStatus) { // Expecting 0x09
      spdlog::warn("readDescriptorNonStandard: Expected ACCEPTED (0x09) but "
                   "got 0x{:02x}. Stopping read.",
                   responseStatus);
      if (accumulatedData.empty()) {
        if (responseStatus == kAVCRejectedStatus)
          return std::unexpected(IOKitError(kIOReturnNotPermitted));
        if (responseStatus == kAVCNotImplementedStatus)
    ...
    // Minimum size needed: Status(1)+Addr(1)+Opcode(1)+Specifier(1) +
    // Metadata(8) + Data(>=0) = 12
    const size_t MIN_RESPONSE_SIZE_NON_STD = 12;
    if (response.size() < MIN_RESPONSE_SIZE_NON_STD) {
      spdlog::warn("readDescriptorNonStandard: ACCEPTED response too short "
                   "({}) for expected metadata. Stopping read.",
                   response.size());
    ...
    if (!lengthDetermined && attempts == 1) {
      ...
      totalExpectedDataLength =
          (static_cast<uint16_t>(response[10]) << 8) | response[11];
      lengthDetermined = true;
      spdlog::info("readDescriptorNonStandard: Determined expected descriptor "
                   "data length: {}",
                   totalExpectedDataLength);
      // If specific length was requested, override totalExpectedDataLength
      if (length > 0) {
        totalExpectedDataLength = length;
        spdlog::info("readDescriptorNonStandard: Requested specific length {}, "
                     "overriding internal length.",
                     length);
      }
      ...
      if (length == 0 && totalExpectedDataLength > 0) {
        try {
          ...
          spdlog::error("readDescriptorNonStandard: Failed to reserve memory "
                        "({} bytes): {}",
                        totalExpectedDataLength, le.what());
          return std::unexpected(IOKitError::NoMemory);
        } catch (...) {
          spdlog::error(
              "readDescriptorNonStandard: Unknown exception during reserve.");
          return std::unexpected(IOKitError::NoMemory);
        }
      }
    }
    ...
    if (response.size() <= dataStartOffset) {
      spdlog::warn("readDescriptorNonStandard: ACCEPTED response has no data "
                   "payload (size {}). Stopping read.",
                   response.size());
      break; // Stop reading
    }
    ...
    if (lengthDetermined) {
      size_t remainingNeeded =
          (totalExpectedDataLength >= accumulatedData.size())
              ? (totalExpectedDataLength - accumulatedData.size())
              : 0;
    ...
    spdlog::trace(
        "  Non-Standard Read: Appending {} bytes from response payload.",
        bytesToAppend);
    if (bytesToAppend > 0) {
      accumulatedData.insert(
          accumulatedData.end(), response.begin() + dataStartOffset,
          response.begin() + dataStartOffset + bytesToAppend);
    ...
    if (lengthDetermined && accumulatedData.size() >= totalExpectedDataLength) {
      spdlog::debug("readDescriptorNonStandard: Reached expected total length "
                    "{}. Stopping.",
                    totalExpectedDataLength);
      if (accumulatedData.size() > totalExpectedDataLength) {
        spdlog::warn("readDescriptorNonStandard: Read slightly more ({}) than "
                     "expected ({}). Trimming.",
                     accumulatedData.size(), totalExpectedDataLength);
  ...
  spdlog::info("readDescriptorNonStandard: Finished. Read {} bytes total.",
               accumulatedData.size());

  // *** CRITICAL: Prepend the 2-byte length field to match what
  // MusicSubunitDescriptorParser expects ***
  if (lengthDetermined) {
    ...
    finalData.insert(finalData.end(), accumulatedData.begin(),
                     accumulatedData.end());
    spdlog::debug("readDescriptorNonStandard: Prepended length 0x{:04X}. Final "
                  "data size: {}",
                  totalExpectedDataLength, finalData.size());
    return finalData;
  } else if (!accumulatedData.empty()) {
    // This case shouldn't happen if the first response always contains the
    // length, but handle defensively
    spdlog::warn(
        "readDescriptorNonStandard: Length was never determined, but data was "
        "read. Prepending length based on accumulated size {}.",
        accumulatedData.size());
    std::vector<uint8_t> finalData;
    uint16_t accumulatedSize16 =
        static_cast<uint16_t>(std::min(accumulatedData.size(), (size_t)0xFFFF));
    ...
    spdlog::error("readDescriptorNonStandard: Failed to determine length and "
                  "read any data.");
    return std::unexpected(
        IOKitError(kIOReturnBadResponse)); // Or the last error recorded
  }
}
// ...existing code...
std::expected<void, IOKitError> DeviceParser::parse() {
  ...
  if (!commandInterface_) {
    spdlog::info("DeviceParser::parse: No CommandInterface available. Using "
                 "basic device capabilities only.");
  ...
  std::unique_ptr<DescriptorAccessor> descriptorAccessor =
      std::make_unique<DescriptorAccessor>(commandInterface_);
  ...
  spdlog::info("DeviceParser: Starting capability parsing for GUID: 0x{:x}",
               device_->getGuid());
  ...
  spdlog::debug("DeviceParser [Phase 1]: Attempting to read Unit Identifier "
                "Descriptor (Addr 0xFF)...");
  {
    std::vector<uint8_t> unitIdSpecifier =
        DescriptorUtils::buildDescriptorSpecifier(
    ...
    if (!openResult) {
      if (openResult.error() == IOKitError::Unsupported) {
        spdlog::info("DeviceParser [Phase 1]: Standard Descriptor Mechanism "
                     "(OPEN) not supported by UNIT 0xFF.");
        // Continue to potentially try subunit open
      } else {
        spdlog::error("DeviceParser [Phase 1]: Failed to OPEN Unit Identifier "
                      "Descriptor: 0x{:x}.",
      ...
      spdlog::debug("DeviceParser [Phase 1]: OPEN Unit Identifier successful. "
                    "Reading...");
      ...
      if (readResult) {
        ...
        if (unitSizes.valid) {
          descriptorMechanismSupported_ =
              true; // Mark as supported *at the unit level*
          descriptorAccessor->updateDescriptorSizes(unitSizes.sizeOfListId,
                                                    unitSizes.sizeOfObjectId,
                                                    unitSizes.sizeOfEntryPos);
          spdlog::info("DeviceParser [Phase 1]: Successfully read Unit ID and "
                       "parsed sizes. Descriptor Mechanism SUPPORTED by Unit.");
        } else {
          spdlog::warn(
              "DeviceParser [Phase 1]: Read Unit ID data but failed to parse "
              "sizes. Descriptor mechanism support uncertain.");
        }
      } else {
        spdlog::warn("DeviceParser [Phase 1]: Failed to READ Unit Identifier "
                     "after successful OPEN: 0x{:x}.",
  ...
  spdlog::debug(
      "DeviceParser [Phase 2]: Discovering subunits and plug counts...");
  ...
  if (info_.hasMusicSubunit() && !descriptorMechanismSupported_) {
    spdlog::info("DeviceParser [Phase 3]: Unit OPEN failed, attempting to OPEN "
                 "Music Subunit Identifier directly (Addr 0x{:02x})...",
                 Helpers::getSubunitAddress(SubunitType::Music,
                                            info_.musicSubunit_.getId()));
    uint8_t musicSubunitAddr = Helpers::getSubunitAddress(
        SubunitType::Music, info_.musicSubunit_.getId());
    // Still use specifier type 0x00 (UnitSubunitIdentifier) but send to subunit
    // address
    std::vector<uint8_t> subunitIdSpecifier =
        DescriptorUtils::buildDescriptorSpecifier(
            DescriptorSpecifierType::UnitSubunitIdentifier, 0, 0, 0);
    triedSubunitOpen = true;

    auto openSubunitResult =
        descriptorAccessor->openForRead(musicSubunitAddr, subunitIdSpecifier);
    if (!openSubunitResult) {
      if (openSubunitResult.error() == IOKitError::Unsupported) {
        spdlog::info("DeviceParser [Phase 3]: Standard Descriptor Mechanism "
                     "(OPEN) also not supported by Music SUBUNIT 0x{:02x}.",
                     musicSubunitAddr);
      } else {
        spdlog::warn("DeviceParser [Phase 3]: Failed to OPEN Music Subunit "
                     "Identifier: 0x{:x}.",
                     static_cast<int>(openSubunitResult.error()));
      }
      info_.musicSubunit_.capabilities_
          .reset(); // Ensure no capabilities if open fails
    } else {
      // Music SUBUNIT OPEN Succeeded! Try to READ its Identifier
      spdlog::debug("DeviceParser [Phase 3]: OPEN Music Subunit Identifier "
                    "successful. Reading...");
      auto readSubunitResult =
          descriptorAccessor->read(musicSubunitAddr, subunitIdSpecifier, 0, 0);
      auto closeSubunitResult =
          descriptorAccessor->close(musicSubunitAddr, subunitIdSpecifier);
      if (!closeSubunitResult) { /* warn */
      }

      if (!readSubunitResult) {
        spdlog::warn(
            "DeviceParser [Phase 3]: Failed to READ Music Subunit Identifier "
            "after successful OPEN: 0x{:x}. Static capabilities unavailable.",
            static_cast<int>(readSubunitResult.error()));
        info_.musicSubunit_.capabilities_.reset();
      } else {
        // Successfully read Music Subunit Identifier! Parse capabilities.
        // We still need the Unit's size definitions if available, otherwise use
        // defaults.
        size_t listIdSizeToUse = unitSizes.valid
                                     ? unitSizes.sizeOfListId
                                     : DescriptorUtils::DEFAULT_SIZE_OF_LIST_ID;
        size_t objIdSizeToUse =
            unitSizes.valid ? unitSizes.sizeOfObjectId
                            : DescriptorUtils::DEFAULT_SIZE_OF_OBJECT_ID;
        size_t entryPosSizeToUse =
            unitSizes.valid ? unitSizes.sizeOfEntryPos
                            : DescriptorUtils::DEFAULT_SIZE_OF_ENTRY_POS;

        auto parsedResult = parseMusicSubunitIdentifier(
            readSubunitResult.value(), listIdSizeToUse, objIdSizeToUse,
            entryPosSizeToUse);
        if (parsedResult) {
          ...
          // spdlog::debug(" -> Stored Dependent Block ({} bytes), Specific
          // Block ({} bytes)", depBlock.size(), specBlock.size());
          spdlog::info(
              "DeviceParser [Phase 3]: Successfully parsed Music Subunit "
              "static capabilities via direct subunit access.");
          // If we got this far, the *subunit* supports descriptors, even if the
          // unit didn't
          descriptorMechanismSupported_ =
              true; // Indicate support at subunit level
          // Update accessor sizes based on subunit's own values if different
          // (though unlikely needed now) uint8_t subGenId =
          // readSubunitResult.value()[2]; size_t subSizeListId =
          // readSubunitResult.value()[3]; size_t subSizeObjId =
          // readSubunitResult.value()[4]; size_t subSizeEntryPos =
          // readSubunitResult.value()[5]; if (subSizeListId != listIdSizeToUse
          // || ...) {
          //     descriptorAccessor.updateDescriptorSizes(subSizeListId,
          //     subSizeObjId, subSizeEntryPos);
          // }
        } else {
          spdlog::warn(
              "DeviceParser [Phase 3]: Failed to parse static capabilities "
              "from Music Subunit Identifier data (read via direct access).");
          info_.musicSubunit_.capabilities_.reset();
        }
      }
    }
  } else if (info_.hasMusicSubunit() && descriptorMechanismSupported_) {
    // Unit open succeeded, attempt to read subunit identifier (as originally
    // planned) This path likely won't be hit for the Duet if Unit OPEN always
    // fails.
    spdlog::debug("DeviceParser [Phase 3]: Unit supports descriptors, "
                  "attempting Music Subunit Identifier read (Addr 0x{:02x})...",
                  Helpers::getSubunitAddress(SubunitType::Music,
                                             info_.musicSubunit_.getId()));
    // ...(Logic to read/parse subunit identifier as before, using accessor with
    // unitSizes)...
    uint8_t musicSubunitAddr = Helpers::getSubunitAddress(
        SubunitType::Music, info_.musicSubunit_.getId());
    std::vector<uint8_t> subunitIdSpecifier =
    ...
    if (!openResult) {
      spdlog::warn(
          "DeviceParser [Phase 3 - Unit OK path]: Failed to OPEN Music Subunit "
          "Identifier (0x{:x}). Cannot read static capabilities.",
          static_cast<int>(openResult.error()));
    } else {
      auto readResult =
          descriptorAccessor->read(musicSubunitAddr, subunitIdSpecifier, 0, 0);
      ...
      if (!readResult) {
        spdlog::warn(
            "DeviceParser [Phase 3 - Unit OK path]: Failed to READ Music "
            "Subunit Identifier (0x{:x}). Cannot read static capabilities.",
            static_cast<int>(readResult.error()));
      } else {
    ...
    if (subunitReadOK) {
      auto parsedResult = parseMusicSubunitIdentifier(
          subunitDescriptorData, unitSizes.sizeOfListId,
          unitSizes.sizeOfObjectId, unitSizes.sizeOfEntryPos);
      if (parsedResult) {
        auto &[caps, depBlock, specBlock] = parsedResult.value();
        info_.musicSubunit_.capabilities_ = std::move(caps);
        // Optionally store the extracted blocks if needed
        // info_.musicSubunit_.setDependentInfoBlock(std::move(depBlock));
        // info_.musicSubunit_.setSpecificInfoBlock(std::move(specBlock));
        // spdlog::debug(" -> Stored Dependent Block ({} bytes), Specific Block
        // ({} bytes)", depBlock.size(), specBlock.size());
        spdlog::info("DeviceParser [Phase 3 - Unit OK path]: Successfully "
                     "parsed Music Subunit static capabilities.");
      } else {
        spdlog::warn("DeviceParser [Phase 3 - Unit OK path]: Failed to parse "
                     "static capabilities from Music Subunit Identifier data.");
        info_.musicSubunit_.capabilities_.reset();
      }
    } else {
      info_.musicSubunit_.capabilities_.reset();
  ...
  // Run ONLY if descriptor mechanism was confirmed working (either at unit or
  // subunit level)
  if (info_.hasMusicSubunit()) {
    spdlog::debug(
        "DeviceParser [Phase 4]: Fetching Music Subunit Status Descriptor...");
    bool statusReadSuccess = false;
    std::vector<uint8_t> statusDescriptorData;
    if (!descriptorMechanismSupported_) { // TODO: remove this check if we
                                          // always support descriptors
      spdlog::debug(
          " -> Attempting standard read via MusicSubunitDescriptorParser...");
      auto result = musicDescParser.fetchAndParse(info_.musicSubunit_);
      if (result) {
        spdlog::info(
            "DeviceParser [Phase 4]: Successfully parsed Music Subunit Status "
            "Descriptor Info Blocks via standard mechanism.");
        statusReadSuccess = true;
      } else {
        spdlog::warn("DeviceParser [Phase 4 WARNING]: Standard read/parse of "
                     "Music Subunit Status Descriptor failed: 0x{:x}.",
                     static_cast<int>(result.error()));
      }
    } else {
      spdlog::info(
          "DeviceParser [Phase 4]: Skipping standard Music Subunit Status "
          "Descriptor read because Descriptor Mechanism not supported.");
    }
    if (!statusReadSuccess) {
      spdlog::warn(
          "DeviceParser [Phase 4]: Standard Status Descriptor read failed or "
          "skipped. Attempting non-standard CONTROL read...");
      uint8_t musicSubunitAddr = Helpers::getSubunitAddress(
          SubunitType::Music, info_.musicSubunit_.getId());
      std::vector<uint8_t> statusDescSpecifier = {static_cast<uint8_t>(
          0x80)}; // 0x80 for Music Subunit Status Descriptor
      auto nonStdReadResult = readDescriptorNonStandard(
          commandInterface_, musicSubunitAddr, statusDescSpecifier, 0, 0);
      if (nonStdReadResult) {
        statusDescriptorData = nonStdReadResult.value();
        auto parseResult = musicDescParser.parseMusicSubunitStatusDescriptor(
            statusDescriptorData, info_.musicSubunit_);
        if (parseResult) {
          spdlog::info(
              "DeviceParser [Phase 4]: Successfully parsed Music Subunit "
              "Status Descriptor Info Blocks via NON-STANDARD read.");
          statusReadSuccess = true;
        } else {
          spdlog::error("DeviceParser [Phase 4]: Failed to parse data obtained "
                        "from NON-STANDARD read: 0x{:x}",
                        static_cast<int>(parseResult.error()));
          info_.musicSubunit_.setStatusDescriptorData(statusDescriptorData);
        }
      } else {
        spdlog::error("DeviceParser [Phase 4]: Non-standard read also failed: "
                      "0x{:x}. Status info blocks unavailable.",
                      static_cast<int>(nonStdReadResult.error()));
  ...
  if (auto result = parseUnitPlugs(plugDetailParser, info_);
      !result) { /* warn */
  }
  if (auto result = parseSubunitPlugs(plugDetailParser, info_);
      !result) { /* warn */
  }

  spdlog::info("DeviceParser: Capability parsing finished for GUID: 0x{:x}",
               device_->getGuid());
  return {}; // Return success
}

std::expected<void, IOKitError>
DeviceParser::parseUnitPlugs(PlugDetailParser &plugDetailParser,
                             DeviceInfo &info) {
  // ... (Keep existing implementation for now) ...
  // This uses PlugDetailParser, which uses CommandInterface.
  // It might need updates later if plug details depend on parsed descriptor
  // sizes.
  ...
  for (uint8_t i = 0; i < info.getNumIsoInputPlugs(); ++i) {
    auto plugResult = plugDetailParser.parsePlugDetails(
        0xFF, i, PlugDirection::Input, PlugUsage::Isochronous);
    if (plugResult) {
      info.isoInputPlugs_.push_back(plugResult.value());
    } else {
      spdlog::warn("Failed to parse details for Iso In plug {}: 0x{:x}", i,
                   static_cast<int>(plugResult.error()));
    }
  }
  ...
  for (uint8_t i = 0; i < info.getNumIsoOutputPlugs(); ++i) {
    auto plugResult = plugDetailParser.parsePlugDetails(
        0xFF, i, PlugDirection::Output, PlugUsage::Isochronous);
    if (plugResult) {
      info.isoOutputPlugs_.push_back(plugResult.value());
    } else {
      spdlog::warn("Failed to parse details for Iso Out plug {}: 0x{:x}", i,
                   static_cast<int>(plugResult.error()));
    }
  }
  // ... (parse external plugs similarly) ...
  spdlog::debug("Parsing Unit External Plugs (In: {}, Out: {})...",
                info.getNumExternalInputPlugs(),
                info.getNumExternalOutputPlugs());
  ...
  for (uint8_t i = 0; i < info.getNumExternalInputPlugs(); ++i) {
    ...
    auto plugResult = plugDetailParser.parsePlugDetails(
        0xFF, plugNum, PlugDirection::Input, PlugUsage::External);
    if (plugResult) {
      info.externalInputPlugs_.push_back(plugResult.value());
    } else {
      spdlog::warn("Failed to parse details for External In plug {}: 0x{:x}", i,
                   static_cast<int>(plugResult.error()));
    }
  }
  ...
  for (uint8_t i = 0; i < info.getNumExternalOutputPlugs(); ++i) {
    uint8_t plugNum = 0x80 + i;
    auto plugResult = plugDetailParser.parsePlugDetails(
        0xFF, plugNum, PlugDirection::Output, PlugUsage::External);
    if (plugResult) {
...
std::expected<void, IOKitError>
DeviceParser::parseSubunitPlugs(PlugDetailParser &plugDetailParser,
                                DeviceInfo &info) {
  // ... (Keep existing implementation for now) ...
  // This uses PlugDetailParser, which uses CommandInterface.
  // It might need updates later if plug details depend on parsed descriptor
  ...
  if (info.hasMusicSubunit()) {
    ...
    uint8_t musicSubunitAddr =
        FWA::Helpers::getSubunitAddress(music.getSubunitType(), music.getId());
    for (uint8_t i = 0; i < music.getMusicDestPlugCount(); ++i) {
      auto plugResult = plugDetailParser.parsePlugDetails(
          musicSubunitAddr, i, PlugDirection::Input, PlugUsage::MusicSubunit);
      if (plugResult) {
    ...
    for (uint8_t i = 0; i < music.getMusicSourcePlugCount(); ++i) {
      auto plugResult = plugDetailParser.parsePlugDetails(
          musicSubunitAddr, i, PlugDirection::Output, PlugUsage::MusicSubunit);
      if (plugResult) {
  ...
  if (info.hasAudioSubunit()) {
    ...
    uint8_t audioSubunitAddr =
        FWA::Helpers::getSubunitAddress(audio.getSubunitType(), audio.getId());
    for (uint8_t i = 0; i < audio.getAudioDestPlugCount(); ++i) {
      auto plugResult = plugDetailParser.parsePlugDetails(
          audioSubunitAddr, i, PlugDirection::Input, PlugUsage::AudioSubunit);
      if (plugResult) {
    ...
    for (uint8_t i = 0; i < audio.getAudioSourcePlugCount(); ++i) {
      auto plugResult = plugDetailParser.parsePlugDetails(
          audioSubunitAddr, i, PlugDirection::Output, PlugUsage::AudioSubunit);
      if (plugResult) {
