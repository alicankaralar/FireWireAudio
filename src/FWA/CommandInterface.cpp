...
CommandInterface::CommandInterface(std::shared_ptr<AudioDevice> pAudioDevice) :
    ...
    refCon_(nullptr),
    protocol_(DeviceProtocol::UNKNOWN),
    diceInterface_(nullptr)
{
    ...
    else
    {
        ...
        {
            spdlog::warn(
                "CommandInterface: Unknown protocol - device is neither AVC "
                "nor DICE");
        }
    }
}
...
std::expected<void, IOKitError> CommandInterface::createAVCUnitInterface()
{
    ...
    {
        spdlog::warn(
            "CommandInterface::createAVCUnitInterface: Cannot create "
            "interface, avcUnit_ is null (Device might not be "
            "an AVC Unit).");
        return std::unexpected(IOKitError(kIOReturnUnsupported)); // Or kIOReturnNotFound? Unsupported
                                                                  // seems better.
    }
    // --- End check ---
    ...
    IOReturn result
        = IOCreatePlugInInterfaceForService(avcUnit_, kIOFireWireAVCLibUnitTypeID, kIOCFPlugInInterfaceID, &plugInInterface, &score);
    ...
    HRESULT comResult
        = (*plugInInterface)
              ->QueryInterface(plugInInterface, CFUUIDGetUUIDBytes(kIOFireWireAVCLibUnitInterfaceID_v2), (void **)&avcInterface_);
...
std::expected<void, IOKitError> CommandInterface::createDICEInterface()
{
    ...
    spdlog::debug("CommandInterface::createDICEInterface: Creating DICE interface");

    // Get the FireWire device interface from the AudioDevice
    io_service_t fwDevice = pAudioDevice_->getFireWireDevice();
    if (fwDevice == 0)
    {
        spdlog::error("CommandInterface::createDICEInterface: No FireWire device available");
...
std::expected<std::vector<uint8_t>, IOKitError> CommandInterface::sendCommand(const std::vector<uint8_t>& command)
{
    ...
    {
        spdlog::error("CommandInterface::sendCommand: No supported protocol available");
        return std::unexpected(static_cast<IOKitError>(kIOReturnUnsupported));
    }
}

std::expected<std::vector<uint8_t>, IOKitError> CommandInterface::sendAVCCommand(const std::vector<uint8_t>& command)
{
    if (!avcInterface_)
    {
        spdlog::error(
            "CommandInterface::sendCommand: Attempted to send command "
    ...
    // --- Enhanced Logging ---
    spdlog::debug(
        "CommandInterface::sendCommand: AVCCommand call returned "
        "IOReturn: 0x{:x} ({})",
        result,
        result);
    ...
    {                     // actualRespLen > respCapacity (Shouldn't happen if API works
                          // correctly)
        response.clear(); // Clear response as it's invalid
        spdlog::error(
            " --> RSP Error: Invalid actualRespLen ({}) returned, "
            "exceeds capacity ({}).",
            actualRespLen,
            respCapacity);
    ...
    {
        spdlog::error(
            "CommandInterface::sendCommand: Error sending command "
            "(IOReturn: 0x{:x})",
            result);
...
std::expected<std::vector<uint8_t>, IOKitError> CommandInterface::sendDICECommand(const std::vector<uint8_t>& command)
{
    if (!diceInterface_)
    {
        spdlog::error(
    ...
    if (command.size() >= 4)
    {
        ...
        address = (command[0] << 24) | (command[1] << 16) | (command[2] << 8) | command[3];
    }
    ...
    if (result == kIOReturnSuccess)
    {
        ...
        spdlog::debug(" --> DICE RSP Bytes ({}): {}", response.size(), Helpers::formatHexBytes(response));
    }
    else
    {
        spdlog::error(
            "CommandInterface::sendDICECommand: Error sending command "
            "(IOReturn: 0x{:x})",
            result);
        return std::unexpected(static_cast<IOKitError>(result));
    }
...
void CommandInterface::deviceInterestCallback(void *refCon, io_service_t service, natural_t messageType, void *messageArgument)
{
