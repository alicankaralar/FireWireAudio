...
namespace FWA {
          ...
          diceInterface_(nullptr)
    {
        ...
        else
        {
            ...
            {
                spdlog::warn("CommandInterface: Unknown protocol - device is neither AVC nor DICE");
            }
        }
    }
...
std::expected<void, IOKitError> CommandInterface::createDICEInterface()
{
    ...
    // This is a placeholder - you'll need to implement the actual interface creation
...
std::expected<void, IOKitError> CommandInterface::releaseDICEInterface()
{
    if (diceInterface_)
    {
        ...
        // This is a placeholder - you'll need to implement the actual interface release
...
CommandInterface::sendDICECommand(const std::vector<uint8_t> &command)
{
    if (!diceInterface_)
    {
        spdlog::error("CommandInterface::sendDICECommand: Attempted to send command "
                      "but diceInterface_ is null.");
        return std::unexpected(static_cast<IOKitError>(kIOReturnNotOpen));
    }

    spdlog::debug(
        "CommandInterface::sendDICECommand: Sending DICE command");
    spdlog::debug(" --> CMD Bytes ({}): {}", command.size(),
                  Helpers::formatHexBytes(command));
    ...
    // This is a placeholder implementation - you'll need to implement the actual DICE protocol
    // Based on your specific DICE device's requirements
    ...
    // Example read operation (actual implementation will depend on your DICE device)
    UInt32 readValue = 0;
