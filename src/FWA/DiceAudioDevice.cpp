...
DiceAudioDevice::DiceAudioDevice(std::uint64_t guid,
    ...
    : AudioDevice(guid, deviceName, vendorName, avcUnit, deviceController),
      m_global_reg_offset(0xFFFFFFFF), m_global_reg_size(0xFFFFFFFF),
      m_tx_reg_offset(0xFFFFFFFF), m_tx_reg_size(0xFFFFFFFF),
      m_rx_reg_offset(0xFFFFFFFF), m_rx_reg_size(0xFFFFFFFF),
      m_unused1_reg_offset(0xFFFFFFFF), m_unused1_reg_size(0xFFFFFFFF),
      m_unused2_reg_offset(0xFFFFFFFF), m_unused2_reg_size(0xFFFFFFFF),
      m_nb_tx(0xFFFFFFFF), m_tx_size(0xFFFFFFFF), m_nb_rx(0xFFFFFFFF),
      m_rx_size(0xFFFFFFFF),
      chipType_(static_cast<int>(DICE::DiceChipType::Unknown)), notifier_(0) {
...
std::expected<void, IOKitError> DiceAudioDevice::init() {
  spdlog::info("********** DICE INIT: Initializing DiceAudioDevice (GUID: "
               "0x%llx) **********",
               getGuid());
  ...
  spdlog::info(
      "DICE INIT: Starting initIoFunctions() to set up register access");
  std::expected<void, IOKitError> ioFuncResult = initIoFunctions();
  if (!ioFuncResult) {
    spdlog::error(
        "DICE INIT ERROR: Failed to initialize DICE I/O functions: {}",
        static_cast<int>(ioFuncResult.error()));
  ...
  if (generalCapResult) {
    ...
    spdlog::info("DICE INIT: Read capability register successfully: 0x{:08x}",
                 generalCap);
    ...
    case DICE::DiceChipType::DiceJr:
      ...
      break;
    default:
      spdlog::info("DICE INIT: Unknown DICE chipset type: %d (0x{:08x})",
                   chipTypeValue, generalCap);
      break;
    }
  } else {
    spdlog::warn("DICE INIT WARNING: Could not determine DICE chip type - EAP "
...
int DiceAudioDevice::getChipType()
    const // This one remains const as it doesn't do I/O
{
  return chipType_;
}
...
std::expected<void, IOKitError> DiceAudioDevice::setSampleRate(int sampleRate) {
  ...
  if (!supported) {
    spdlog::error("Sample rate %d Hz is not supported by this device",
                  sampleRate);
    return std::unexpected(IOKitError::Unsupported);
  }
  ...
  if (eap_) {
    ...
    if (!eapUpdate) {
      spdlog::warn("Failed to update EAP: {}",
                   static_cast<int>(eapUpdate.error()));
    }
  }
...
DiceAudioDevice::setNickname(const std::string &name) {
...
std::expected<bool, IOKitError>
DiceAudioDevice::maskedCheckZeroGlobalReg(uint32_t offset, uint32_t mask) {
  std::expected<uint32_t, IOKitError> result = readGlobalReg(offset);
  if (!result) {
    spdlog::error(
        "Could not read global register at offset 0x{:x} for masked check",
        offset);
...
std::expected<bool, IOKitError>
DiceAudioDevice::maskedCheckNotZeroGlobalReg(uint32_t offset, uint32_t mask) {
  std::expected<bool, IOKitError> result =
      maskedCheckZeroGlobalReg(offset, mask);
  if (!result) {
    return std::unexpected(result.error());
  }
  return !result.value();
...
uint64_t DiceAudioDevice::globalOffsetGen(uint32_t offset, size_t length) {
  ...
  if (offset + length > m_global_reg_offset + m_global_reg_size) {
    spdlog::error("Register offset+length too large: 0x{:04X}",
                  offset + length);
    return DICE_INVALID_OFFSET;
  }
...
uint64_t DiceAudioDevice::txOffsetGen(unsigned int i, uint32_t offset,
                                      size_t length) {
  // Registry offsets should always be smaller than 0x7FFFFFFF
  // because otherwise base + offset > 64bit
  ...
  if (m_tx_reg_offset + offset_tx + length > m_tx_reg_offset + m_tx_reg_size) {
    spdlog::error("TX Register offset+length out of bounds: base=0x{:X}, "
...
uint64_t DiceAudioDevice::rxOffsetGen(unsigned int i, uint32_t offset,
                                      size_t length) {
  // Registry offsets should always be smaller than 0x7FFFFFFF
  // because otherwise base + offset > 64bit
  ...
  if (m_rx_reg_offset + offset_rx + length > m_rx_reg_offset + m_rx_reg_size) {
    spdlog::error("RX Register offset+length out of bounds: base=0x{:X}, "
                  "offset=0x{:X}, length={}, total_size=0x{:X}",
...
std::expected<void, IOKitError> DiceAudioDevice::initIoFunctions() {
  spdlog::info("********** DICE IO INIT: Reading DICE register offsets and "
               "sizes **********");
  ...
  spdlog::debug(" Global  : offset=0x{:04X} size={:04d}", m_global_reg_offset,
                m_global_reg_size);
  spdlog::debug(" TX      : offset=0x{:04X} size={:04d}", m_tx_reg_offset,
                m_tx_reg_size);
  spdlog::debug("               nb={:4d} size={:04d}", m_nb_tx, m_tx_size);
  spdlog::debug(" RX      : offset=0x{:04X} size={:04d}", m_rx_reg_offset,
                m_rx_reg_size);
  spdlog::debug("               nb={:4d} size={:04d}", m_nb_rx, m_rx_size);
  spdlog::debug(" UNUSED1 : offset=0x{:04X} size={:04d}", m_unused1_reg_offset,
                m_unused1_reg_size);
  spdlog::debug(" UNUSED2 : offset=0x{:04X} size={:04d}", m_unused2_reg_offset,
                m_unused2_reg_size);
...
DiceAudioDevice::readGlobalReg(uint32_t offset) {
  uint64_t addr = globalOffsetGen(offset, 4);
  if (addr == DICE_INVALID_OFFSET)
    return std::unexpected(
        IOKitError::BadArgument); // Or a more specific error if possible
  return readReg(addr);
}

std::expected<void, IOKitError> DiceAudioDevice::writeGlobalReg(uint32_t offset,
                                                                uint32_t data) {
  uint64_t addr = globalOffsetGen(offset, 4);
  if (addr == DICE_INVALID_OFFSET)
    return std::unexpected(
...
DiceAudioDevice::readGlobalRegBlock(uint32_t offset, size_t length) {
...
std::expected<void, IOKitError>
DiceAudioDevice::writeGlobalRegBlock(uint32_t offset, const uint32_t *data,
                                     size_t length) {
  uint64_t addr = globalOffsetGen(offset, length);
  if (addr == DICE_INVALID_OFFSET)
    return std::unexpected(
...
std::expected<uint32_t, IOKitError>
DiceAudioDevice::readTxReg(unsigned int i, uint32_t offset) {
...
std::expected<void, IOKitError>
DiceAudioDevice::writeTxReg(unsigned int i, uint32_t offset, uint32_t data) {
  uint64_t addr = txOffsetGen(i, offset, 4);
  if (addr == DICE_INVALID_OFFSET)
    return std::unexpected(IOKitError::BadArgument);
...
std::expected<std::vector<uint32_t>, IOKitError>
DiceAudioDevice::readTxRegBlock(unsigned int i, uint32_t offset,
                                size_t length) {
...
std::expected<void, IOKitError>
DiceAudioDevice::writeTxRegBlock(unsigned int i, uint32_t offset,
...
std::expected<uint32_t, IOKitError>
DiceAudioDevice::readRxReg(unsigned int i, uint32_t offset) {
...
std::expected<void, IOKitError>
DiceAudioDevice::writeRxReg(unsigned int i, uint32_t offset, uint32_t data) {
  uint64_t addr = rxOffsetGen(i, offset, 4);
  if (addr == DICE_INVALID_OFFSET)
    return std::unexpected(IOKitError::BadArgument);
...
std::expected<std::vector<uint32_t>, IOKitError>
DiceAudioDevice::readRxRegBlock(unsigned int i, uint32_t offset,
                                size_t length) {
...
std::expected<void, IOKitError>
DiceAudioDevice::writeRxRegBlock(unsigned int i, uint32_t offset,
                                 const uint32_t *data, size_t length) {
  uint64_t addr = rxOffsetGen(i, offset, length);
  if (addr == DICE_INVALID_OFFSET)
    return std::unexpected(IOKitError::BadArgument);
...
std::expected<uint32_t, IOKitError> DiceAudioDevice::readReg(uint64_t offset) {
  spdlog::debug("DiceAudioDevice::readReg - Reading from offset 0x{:x} "
                "[hi:0x{:04x}, lo:0x{:08x}]",
                offset, static_cast<UInt16>((offset >> 32) & 0xFFFF),
                static_cast<UInt32>(offset & 0xFFFFFFFF));

  if (!deviceInterface) {
    spdlog::error("DiceAudioDevice::readReg - deviceInterface is null. Cannot "
  ...
  if (status != kIOReturnSuccess) {
    spdlog::error("DiceAudioDevice::readReg - Failed to get bus generation: "
                  "0x{:x} for offset 0x{:x}",
                  status, offset);
    return std::unexpected(static_cast<IOKitError>(status));
  }
  spdlog::debug("DiceAudioDevice::readReg - Got bus generation: {}",
                generation);

  spdlog::debug("DiceAudioDevice::readReg - Calling ReadQuadlet for offset "
                "0x{:x} [hi:0x{:04x}, lo:0x{:08x}]",
                offset, addr.addressHi, addr.addressLo);
  status = ((IOFireWireDeviceInterface_t *)deviceInterface)
               ->ReadQuadlet(deviceInterface, 0, &addr, &result, kFWFailOnReset,
                             generation);

  if (status != kIOReturnSuccess) {
    spdlog::error("DiceAudioDevice::readReg - ReadQuadlet FAILED for offset "
                  "0x{:x} with status 0x{:x}",
                  offset, status); // Removed IOKitErrorToString call
  ...
  spdlog::debug("DiceAudioDevice::readReg - Successfully read 0x{:08x} from "
                "offset 0x{:x} (byte-swapped: 0x{:08x})",
                result, offset, swappedResult);
  return swappedResult;
}

std::expected<void, IOKitError> DiceAudioDevice::writeReg(uint64_t offset,
                                                          uint32_t data) {
  if (!deviceInterface) {
  ...
  if (status != kIOReturnSuccess) {
    spdlog::error(
        "DiceAudioDevice::writeReg - Failed to get bus generation: 0x{:x}",
        status);
    return std::unexpected(static_cast<IOKitError>(status));
  }
  ...
  status = ((IOFireWireDeviceInterface_t *)deviceInterface)
               ->WriteQuadlet(deviceInterface, 0, &addr, dataToWrite,
                              kFWFailOnReset, generation);

  if (status != kIOReturnSuccess) {
    spdlog::error("DiceAudioDevice::writeReg - WriteQuadlet failed for offset "
                  "0x{:x} with status 0x{:x}",
                  offset, status);
...
std::expected<std::vector<uint32_t>, IOKitError>
DiceAudioDevice::readRegBlock(uint64_t offset, size_t length) {
  if (length % 4 != 0) {
    spdlog::error(
        "DiceAudioDevice::readRegBlock - Length must be a multiple of 4.");
    return std::unexpected(IOKitError::BadArgument);
  }
  ...
  for (size_t i = 0; i < numQuadlets; ++i) {
    ...
    if (!result) {
      spdlog::error("DiceAudioDevice::readRegBlock - Failed reading quadlet {} "
                    "at offset 0x{:x}",
                    i, offset + (i * 4));
      return std::unexpected(result.error());
    }
...
std::expected<void, IOKitError>
DiceAudioDevice::writeRegBlock(uint64_t offset, const uint32_t *data,
                               size_t length) {
  if (length % 4 != 0) {
    spdlog::error(
        "DiceAudioDevice::writeRegBlock - Length must be a multiple of 4.");
    return std::unexpected(IOKitError::BadArgument);
  }
  size_t numQuadlets = length / 4;
  ...
  for (size_t i = 0; i < numQuadlets; ++i) {
    ...
    if (!result) {
      spdlog::error("DiceAudioDevice::writeRegBlock - Failed writing quadlet "
                    "{} at offset 0x{:x}",
                    i, offset + (i * 4));
      return std::unexpected(result.error());
    }
...
std::expected<void, IOKitError> DiceAudioDevice::lock() {
  ...
  if (!ownerHighResult) {
    spdlog::error("Failed to read current owner (high): {}",
                  static_cast<int>(ownerHighResult.error()));
  ...
  if (!ownerLowResult) {
    spdlog::error("Failed to read current owner (low): {}",
                  static_cast<int>(ownerLowResult.error()));
    return std::unexpected(ownerLowResult.error());
  }

  // Combine to form 64-bit owner ID
  currentOwner = (static_cast<uint64_t>(ownerHighResult.value()) << 32) |
                 ownerLowResult.value();

  // Our ownership ID is a unique value that includes our NodeID
  uint32_t nodeID = 0; // We would get this from FireWire API
  uint64_t ourOwnerValue =
      ((static_cast<uint64_t>(0xFFC0 | nodeID)) << 48) | 0x0001;
  ...
  if (deviceInterface) {
    ...
    if (currentOwner == DICE_OWNER_NO_OWNER) {
      ...
      auto writeHighResult =
          writeReg(ownerRegAddr,
                   static_cast<uint32_t>((ourOwnerValue >> 32) & 0xFFFFFFFF));
      if (!writeHighResult) {
      ...
      auto writeLowResult = writeReg(
          ownerRegAddr + 4, static_cast<uint32_t>(ourOwnerValue & 0xFFFFFFFF));
      if (!writeLowResult) {
...
std::expected<void, IOKitError> DiceAudioDevice::unlock() {
  ...
  if (deviceInterface) {
    ...
    auto writeHighResult = writeReg(
        ownerRegAddr, static_cast<uint32_t>((noOwnerValue >> 32) & 0xFFFFFFFF));
    if (!writeHighResult) {
    ...
    auto writeLowResult = writeReg(
        ownerRegAddr + 4, static_cast<uint32_t>(noOwnerValue & 0xFFFFFFFF));
    if (!writeLowResult) {
...
DiceAudioDevice::splitNameString(const std::string &in) {
