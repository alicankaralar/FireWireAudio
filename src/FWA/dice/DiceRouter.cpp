...
DiceRouter::DiceRouter(DiceEAP &eap) : eap_(eap), pointsCached_(false) {
  spdlog::debug("DiceRouter constructor");
}

DiceRouter::~DiceRouter() { spdlog::debug("DiceRouter destructor"); }
...
std::expected<void, IOKitError> DiceRouter::update() {
  ...
  if (!result) {
    spdlog::error("Failed to update EAP state: {}",
...
std::vector<DiceRouter::RoutePoint> DiceRouter::getSources() const {
  ...
  for (int srcId = 0; srcId < static_cast<int>(DICE::RouteSource::Invalid);
       srcId++) {
...
std::vector<DiceRouter::RoutePoint> DiceRouter::getDestinations() const {
  ...
  for (int dstId = 0; dstId < static_cast<int>(DICE::RouteDestination::Invalid);
       dstId++) {
...
std::expected<void, IOKitError> DiceRouter::connect(uint8_t sourceId,
  ...
  if (!result) {
    spdlog::error("Failed to setup route: {}",
                  static_cast<int>(result.error()));
    return std::unexpected(result.error());
  }
...
DiceRouter::connect(const RoutePoint &source, const RoutePoint &destination) {
  return connect(source.id, source.channel, destination.id,
                 destination.channel);
}

std::expected<void, IOKitError> DiceRouter::disconnect(uint8_t destId,
                                                       uint8_t destChannel) {
  spdlog::debug("Disconnecting destination {}:{}", static_cast<int>(destId),
                static_cast<int>(destChannel));
  ...
  if (!result) {
    spdlog::error("Failed to remove route: {}",
                  static_cast<int>(result.error()));
    return std::unexpected(result.error());
  }
...
std::expected<void, IOKitError>
DiceRouter::disconnect(const RoutePoint &destination) {
  return disconnect(destination.id, destination.channel);
}

std::expected<void, IOKitError> DiceRouter::mute(uint8_t destId,
                                                 uint8_t destChannel) {
  spdlog::debug("Muting destination {}:{}", static_cast<int>(destId),
                static_cast<int>(destChannel));
...
std::expected<void, IOKitError>
DiceRouter::mute(const RoutePoint &destination) {
...
std::expected<void, IOKitError> DiceRouter::clearAllRoutes() {
  ...
  if (!result) {
    spdlog::error("Failed to clear routes: {}",
                  static_cast<int>(result.error()));
    return std::unexpected(result.error());
  }
...
std::expected<DiceRouter::RoutePoint, IOKitError>
DiceRouter::getSource(uint8_t destId, uint8_t destChannel) const {
...
std::expected<DiceRouter::RoutePoint, IOKitError>
DiceRouter::getSource(const RoutePoint &destination) const {
  return getSource(destination.id, destination.channel);
}

std::expected<std::vector<DiceRouter::RoutePoint>, IOKitError>
DiceRouter::getDestinations(uint8_t sourceId, uint8_t sourceChannel) const {
...
std::expected<std::vector<DiceRouter::RoutePoint>, IOKitError>
DiceRouter::getDestinations(const RoutePoint &source) const {
...
std::vector<std::vector<bool>> DiceRouter::getRoutingMatrix() const {
  ...
  std::vector<std::vector<bool>> matrix(
      sources.size(), std::vector<bool>(destinations.size(), false));
  ...
  for (const auto &route : routes) {
    ...
    auto srcIt = std::find_if(
        sources.begin(), sources.end(), [&route](const RoutePoint &p) {
    ...
    auto dstIt = std::find_if(destinations.begin(), destinations.end(),
                              [&route](const RoutePoint &p) {
                                return p.id == route.destination.id &&
                                       p.channel == route.destination.channel;
                              });
...
DiceRouter::LabeledMatrix DiceRouter::getLabeledRoutingMatrix() const {
  ...
  for (const auto &src : sources) {
    result.rowLabels.push_back(
        src.name + " [" + std::to_string(static_cast<int>(src.id)) + ":" +
        std::to_string(static_cast<int>(src.channel)) + "]");
  }

  for (const auto &dst : destinations) {
    result.columnLabels.push_back(
        dst.name + " [" + std::to_string(static_cast<int>(dst.id)) + ":" +
...
std::expected<DiceRouter::RoutePoint, IOKitError>
DiceRouter::getMidiInput(unsigned int port) const {
  ...
  return createDestinationPoint(
      static_cast<uint8_t>(DICE::RouteDestination::Mixer1), port);
}

std::expected<DiceRouter::RoutePoint, IOKitError>
DiceRouter::getMidiOutput(unsigned int port) const {
  ...
  // This is a simplification - actual implementation depends on device
  return createSourcePoint(static_cast<uint8_t>(DICE::RouteSource::Mixer),
                           port + 16);
}

std::expected<void, IOKitError>
DiceRouter::setMidiInput(unsigned int port, const RoutePoint &src) {
...
std::expected<void, IOKitError>
DiceRouter::setMidiOutput(unsigned int port, const RoutePoint &dest) {
...
bool DiceRouter::isMidiPort(const RoutePoint &point) const {
  ...
  if (point.id == static_cast<uint8_t>(DICE::RouteDestination::Mixer1) &&
      point.channel < 4) {
    return true;
  }

  // Check if it's a MIDI output (source)
  if (point.id == static_cast<uint8_t>(DICE::RouteSource::Mixer) &&
      point.channel >= 16 && point.channel < 20) {
    return true;
  }
...
std::vector<DiceRouter::RoutePoint> DiceRouter::getAllMidiInputs() const {
  ...
  for (unsigned int i = 0; i < 4; i++) {
    ...
    if (inputResult) {
      ...
      point.name =
          "MIDI Input " + std::to_string(i + 1) + " (" + point.name + ")";
  ...
  for (int ch = 8; ch < 12; ch++) {
    ...
    point.name =
        "FW MIDI Input " + std::to_string(ch - 7) + " (" + point.name + ")";
    midiInputs.push_back(point);
  }
...
std::vector<DiceRouter::RoutePoint> DiceRouter::getAllMidiOutputs() const {
  ...
  for (unsigned int i = 0; i < 4; i++) {
    ...
    if (outputResult) {
      ...
      point.name =
          "MIDI Output " + std::to_string(i + 1) + " (" + point.name + ")";
  ...
  for (int ch = 8; ch < 12; ch++) {
    RoutePoint point =
        createSourcePoint(static_cast<uint8_t>(DICE::RouteSource::InS0), ch);
    point.name =
        "FW MIDI Output " + std::to_string(ch - 7) + " (" + point.name + ")";
    midiOutputs.push_back(point);
  }
...
std::expected<void, IOKitError> DiceRouter::setupDefaultMidiRoutes() {
  ...
  for (size_t i = 0;
       i < std::min(midiInputs.size(), midiOutputs.size()) && i < 4; i++) {
    // Connect physical MIDI input to FireWire MIDI output
    auto result = connect(midiInputs[i], midiOutputs[i]);
    if (!result) {
      spdlog::error("Failed to connect MIDI input {} to output {}: {}", i, i,
                    static_cast<int>(result.error()));
      return std::unexpected(result.error());
    }
    ...
    if (i + 4 < midiInputs.size() && i + 4 < midiOutputs.size()) {
      ...
      if (!result) {
        spdlog::error("Failed to connect FW MIDI input {} to output {}: {}", i,
...
DiceRouter::RoutePoint DiceRouter::createSourcePoint(uint8_t srcId,
...
DiceRouter::RoutePoint
DiceRouter::createDestinationPoint(uint8_t destId, uint8_t channel) const {
  std::string name;
...
std::expected<void, IOKitError> DiceRouter::setTransmitChannelCount(int count) {
  ...
  spdlog::warn("Setting transmit channel count directly is not supported. Use "
               "applyChannelFormat instead.");
...
std::expected<void, IOKitError> DiceRouter::setReceiveChannelCount(int count) {
  ...
  // This is a simplified implementation - real implementation would
  // need to store the new count in the stream configuration
  spdlog::warn("Setting receive channel count directly is not supported. Use "
               "applyChannelFormat instead.");
  return std::unexpected(IOKitError::Unsupported);
}

std::vector<std::pair<int, int>>
DiceRouter::getAvailableChannelFormats() const {
...
std::expected<void, IOKitError> DiceRouter::applyChannelFormat(int txChannels,
                                                               int rxChannels) {
  spdlog::debug("Applying channel format: {} TX, {} RX", txChannels,
                rxChannels);
  ...
  if (!validFormat) {
    spdlog::error("Invalid channel format: {} TX, {} RX", txChannels,
                  rxChannels);
    return std::unexpected(IOKitError::BadArgument);
  }
  ...
  if (!txResult) {
    spdlog::error("Failed to set transmit channel count: {}",
                  static_cast<int>(txResult.error()));
  ...
  if (!rxResult) {
    spdlog::error("Failed to set receive channel count: {}",
                  static_cast<int>(rxResult.error()));
