...
FWA::DiceRouterConfig::DiceRouterConfig(DiceEAP &eap) : eap_(eap) {}
...
std::expected<void, IOKitError> DiceRouterConfig::read(uint32_t offset) {
  ...
  std::expected<void, IOKitError> nbRoutesRes =
      eap_.readReg(DiceEAP::RegBase::CurrentCfg, offset, &nbRoutes);
  if (!nbRoutesRes) {
  ...
  std::expected<void, IOKitError> routesDataRes =
      eap_.readRegBlock(DiceEAP::RegBase::CurrentCfg, offset + 4,
                        routesData.data(), nbRoutes * 4);
  if (!routesDataRes) {
    spdlog::error("Failed to read route configuration data");
    return std::unexpected(routesDataRes.error());
  }

  // Parse routes - format is destination in the lower byte, source in the next
  // byte
  for (uint32_t i = 0; i < nbRoutes; i++) {
    unsigned char dest = routesData[i] & 0xFF;
    unsigned char src = (routesData[i] >> 8) & 0xFF;
    routes_[dest] = src;
    spdlog::debug("Read route: Source 0x{:02x} -> Destination 0x{:02x}", src,
                  dest);
  }
...
std::expected<void, IOKitError> DiceRouterConfig::write(uint32_t offset) {
  ...
  if (nbRoutes > 128) {
    spdlog::warn("More than 128 routes defined ({}) - truncating to 128",
                 nbRoutes);
    nbRoutes = 128;
  }
  ...
  for (const auto &route : routes_) {
    ...
    routesData[idx++] =
        (static_cast<uint32_t>(route.second) << 8) | route.first;
  }
  ...
  std::expected<void, IOKitError> clearRes = eap_.writeRegBlock(
      DiceEAP::RegBase::CurrentCfg, offset, zeros.data(), zeros.size() * 4);
  if (!clearRes) {
  ...
  if (nbRoutes > 0) {
    std::expected<void, IOKitError> routesRes =
        eap_.writeRegBlock(DiceEAP::RegBase::CurrentCfg, offset + 4,
                           routesData.data(), routesData.size() * 4);
    if (!routesRes) {
...
DiceRouterConfig::createRoute(unsigned char srcId, unsigned char dstId) {
...
std::expected<void, IOKitError>
DiceRouterConfig::setupRoute(unsigned char srcId, unsigned char dstId) {
...
std::expected<void, IOKitError>
DiceRouterConfig::muteRoute(unsigned char dstId) {
...
std::expected<void, IOKitError>
DiceRouterConfig::removeRoute(unsigned char dstId) {
  spdlog::debug("Removing route for destination {}", dstId);
  routes_.erase(dstId);
  return {};
}

std::expected<unsigned char, IOKitError>
DiceRouterConfig::getSourceForDestination(unsigned char dstId) {
...
std::expected<std::vector<unsigned char>, IOKitError>
DiceRouterConfig::getDestinationsForSource(unsigned char srcId) {
...
FWA::DicePeakSpace::DicePeakSpace(DiceEAP &eap) : eap_(eap) {}
...
std::expected<void, IOKitError> DicePeakSpace::read(uint32_t offset) {
  ...
  std::expected<void, IOKitError> peakDataRes = eap_.readRegBlock(
      DiceEAP::RegBase::Peak, offset, peakData.data(), numRoutes * 4);
  if (!peakDataRes) {
    spdlog::error("Failed to read peak data");
    return std::unexpected(peakDataRes.error());
  }

  // Parse peak data - format is destination in the lower byte, peak value in
  // bits 16-27
  for (size_t i = 0; i < numRoutes; i++) {
    unsigned char dest = peakData[i] & 0xFF;
    int peakValue = (peakData[i] & 0x0FFF0000) >> 16;

    // Only update if the new peak is higher than any existing peak for this
    // destination
...
std::expected<std::map<unsigned char, int>, IOKitError>
DicePeakSpace::getPeaks() {
...
std::expected<void, IOKitError> DiceStandaloneConfig::read() {
  ...
  std::expected<void, IOKitError> result = eap_.readRegBlock(
      DiceEAP::RegBase::Standalone, 0, data.data(), data.size() * 4);
  if (!result) {
...
std::expected<void, IOKitError> DiceStandaloneConfig::write() {
  ...
  std::vector<uint32_t> data = {clockSrc_, aesExt_, adatExt_, wcExt_, intExt_};

  // Write the configuration
  std::expected<void, IOKitError> result = eap_.writeRegBlock(
      DiceEAP::RegBase::Standalone, 0, data.data(), data.size() * 4);
  if (!result) {
...
FWA::DiceMixer::DiceMixer(DiceEAP &eap) : eap_(eap) {}
...
std::expected<void, IOKitError> DiceMixer::loadCoefficients() {
  ...
  auto result = eap_.readRegBlock(DiceEAP::RegBase::Mixer, 4,
                                  coefficients_.data(), totalSize);
  if (!result) {
...
std::expected<void, IOKitError> DiceMixer::storeCoefficients() {
  ...
  auto result = eap_.writeRegBlock(DiceEAP::RegBase::Mixer, 4,
                                   coefficients_.data(), totalSize);
  if (!result) {
...
int DiceMixer::getRowCount() const { return eap_.mixerNumTx_; }

int DiceMixer::getColCount() const { return eap_.mixerNumRx_; }

bool DiceMixer::canWrite(int row, int col) const {
  if (eap_.mixerReadonly_) {
    return false;
  }
  return (row >= 0 && row < eap_.mixerNumTx_ && col >= 0 &&
          col < eap_.mixerNumRx_);
}

std::expected<double, IOKitError> DiceMixer::setValue(int row, int col,
                                                      double value) {
  if (eap_.mixerReadonly_) {
    spdlog::warn("Mixer is read-only");
    return std::unexpected(IOKitError::ReadOnly);
  ...
  auto result = eap_.writeRegBlock(DiceEAP::RegBase::Mixer,
                                   4 + (index * sizeof(uint32_t)), &intValue,
                                   sizeof(uint32_t));
  if (!result) {
...
std::expected<double, IOKitError> DiceMixer::getValue(int row, int col) {
  ...
  uint32_t value;
  auto result =
      eap_.readRegBlock(DiceEAP::RegBase::Mixer, 4 + (index * sizeof(uint32_t)),
...
std::string DiceMixer::getRowName(int row) const {
  ...
  unsigned int destId =
      ((static_cast<unsigned int>(DICE::RouteDestination::Mixer0) << 4) + row);

  // Create a combined name that includes the source routed to this mixer input
  std::string mixerInputName =
      eap_.getDestinationName(DICE::RouteDestination::Mixer0, row);
...
std::string DiceMixer::getColName(int col) const {
  ...
  unsigned int srcId =
      ((static_cast<unsigned int>(DICE::RouteSource::Mixer) << 4) + col);

  // Create a combined name that includes destinations this mixer output is
  // routed to
  std::string mixerOutputName =
      eap_.getSourceName(DICE::RouteSource::Mixer, col);
    ...
    if (destsResult) {
      ...
      if (!destinations.empty()) {
        ...
        for (unsigned char dest : destinations) {
          ...
          if (dstId <
              static_cast<unsigned char>(DICE::RouteDestination::Invalid)) {
            if (!first) {
              mixerOutputName += ", ";
            }
            DICE::RouteDestination routeDest =
                static_cast<DICE::RouteDestination>(dstId);
...
std::expected<void, IOKitError> DiceStreamConfig::read(uint32_t offset) {
  ...
  std::expected<void, IOKitError> nbTxRes =
      eap_.readReg(DiceEAP::RegBase::CurrentCfg, offset, &numTx_);
  if (!nbTxRes) {
    spdlog::error("Failed to read number of TX streams");
    return std::unexpected(nbTxRes.error());
  }

  std::expected<void, IOKitError> nbRxRes =
      eap_.readReg(DiceEAP::RegBase::CurrentCfg, offset + 4, &numRx_);
  if (!nbRxRes) {
  ...
  if (numTx_ > 0) {
    ...
    for (uint32_t i = 0; i < numTx_; i++) {
      ...
      std::expected<void, IOKitError> cfgRes = eap_.readRegBlock(
          DiceEAP::RegBase::CurrentCfg, configOffset,
      ...
      spdlog::debug("TX stream {}: Audio={}, MIDI={}", i,
                    txConfigs_[i].numAudio, txConfigs_[i].numMidi);
    }
  }
  ...
  if (numRx_ > 0) {
    ...
    for (uint32_t i = 0; i < numRx_; i++) {
      uint32_t configOffset = offset + 8 + (numTx_ * sizeof(ConfigBlock)) +
                              (i * sizeof(ConfigBlock));
      std::expected<void, IOKitError> cfgRes = eap_.readRegBlock(
          DiceEAP::RegBase::CurrentCfg, configOffset,
      ...
      spdlog::debug("RX stream {}: Audio={}, MIDI={}", i,
                    rxConfigs_[i].numAudio, rxConfigs_[i].numMidi);
    }
  }
...
std::expected<void, IOKitError> DiceStreamConfig::write(uint32_t offset) {
  ...
  std::expected<void, IOKitError> nbTxRes =
      eap_.writeReg(DiceEAP::RegBase::NewStreamCfg, offset, numTx_);
  if (!nbTxRes) {
  ...
  std::expected<void, IOKitError> nbRxRes =
      eap_.writeReg(DiceEAP::RegBase::NewStreamCfg, offset + 4, numRx_);
  if (!nbRxRes) {
  ...
  for (uint32_t i = 0; i < numTx_; i++) {
    ...
    std::expected<void, IOKitError> cfgRes =
        eap_.writeRegBlock(DiceEAP::RegBase::NewStreamCfg, configOffset,
                           reinterpret_cast<const uint32_t *>(&txConfigs_[i]),
                           sizeof(ConfigBlock));
    if (!cfgRes) {
    ...
    spdlog::debug("Wrote TX stream {}: Audio={}, MIDI={}", i,
                  txConfigs_[i].numAudio, txConfigs_[i].numMidi);
  }
  ...
  for (uint32_t i = 0; i < numRx_; i++) {
    uint32_t configOffset =
        offset + 8 + (numTx_ * sizeof(ConfigBlock)) + (i * sizeof(ConfigBlock));
    std::expected<void, IOKitError> cfgRes =
        eap_.writeRegBlock(DiceEAP::RegBase::NewStreamCfg, configOffset,
                           reinterpret_cast<const uint32_t *>(&rxConfigs_[i]),
                           sizeof(ConfigBlock));
    if (!cfgRes) {
    ...
    spdlog::debug("Wrote RX stream {}: Audio={}, MIDI={}", i,
                  rxConfigs_[i].numAudio, rxConfigs_[i].numMidi);
  }
...
std::expected<std::vector<std::string>, IOKitError>
DiceStreamConfig::getTxNames(unsigned int index) {
  if (index >= numTx_) {
...
std::expected<std::vector<std::string>, IOKitError>
DiceStreamConfig::getRxNames(unsigned int index) {
  if (index >= numRx_) {
...
DiceStreamConfig::getNamesForBlock(const ConfigBlock &block) {
  ...
  std::memcpy(nameBuffer, block.names,
              DICE_EAP_CHANNEL_CONFIG_NAMESTR_LEN_BYTES);
  ...
  while (*current) {
    ...
    while (*current == '\0' &&
           current < nameBuffer + DICE_EAP_CHANNEL_CONFIG_NAMESTR_LEN_BYTES) {
      current++;
    }
  }
...
DiceEAP::DiceEAP(DiceAudioDevice &device)
    : device_(device), routerExposed_(false), routerReadonly_(false),
      routerFlashstored_(false), routerNumEntries_(0), mixerExposed_(false),
      mixerReadonly_(false), mixerFlashstored_(false), mixerTxId_(0),
      mixerRxId_(0), mixerNumTx_(0), mixerNumRx_(0),
      generalSupportDynstream_(false), generalSupportFlash_(false),
      generalPeakEnabled_(false), generalMaxTx_(0), generalMaxRx_(0),
      generalStreamCfgStored_(false), generalChip_(0), capabilityOffset_(0),
      capabilitySize_(0), cmdOffset_(0), cmdSize_(0), mixerOffset_(0),
      mixerSize_(0), peakOffset_(0), peakSize_(0), newRoutingOffset_(0),
      newRoutingSize_(0), newStreamCfgOffset_(0), newStreamCfgSize_(0),
      currCfgOffset_(0), currCfgSize_(0), standaloneOffset_(0),
      standaloneSize_(0), appOffset_(0), appSize_(0) {
  spdlog::debug("DiceEAP constructor");
}

DiceEAP::~DiceEAP() { spdlog::debug("DiceEAP destructor"); }
...
bool DiceEAP::supportsEAP(DiceAudioDevice &device) {
  ...
  std::expected<uint32_t, IOKitError> res =
      device.readReg(DICE_EAP_BASE + DICE_EAP_CAPABILITY_SPACE_OFFSET);
  if (!res) {
    spdlog::debug(
        "Device does not support EAP: Could not read capability offset");
    return false;
  }

  // Also check for space size
  std::expected<uint32_t, IOKitError> sizeRes =
      device.readReg(DICE_EAP_BASE + DICE_EAP_CAPABILITY_SPACE_SIZE);
  if (!sizeRes) {
    spdlog::debug(
        "Device does not support EAP: Could not read capability size");
    return false;
  }

  spdlog::debug("Device supports EAP: Capability offset=0x{:x}, size=0x{:x}",
                res.value(), sizeRes.value());
  return true;
}
...
std::expected<void, IOKitError> DiceEAP::init() {
  ...
  res = device_.readReg(DICE_EAP_BASE + DICE_EAP_CAPABILITY_SPACE_OFFSET);
  if (!res) {
  ...
  res = device_.readReg(DICE_EAP_BASE + DICE_EAP_CAPABILITY_SPACE_SIZE);
  if (!res) {
  ...
  res = device_.readReg(DICE_EAP_BASE + DICE_EAP_CMD_SPACE_OFFSET);
  if (!res) {
  ...
  res = device_.readReg(DICE_EAP_BASE + DICE_EAP_CMD_SPACE_SIZE);
  if (!res) {
  ...
  res = device_.readReg(DICE_EAP_BASE + DICE_EAP_MIXER_SPACE_OFFSET);
  if (!res) {
  ...
  res = device_.readReg(DICE_EAP_BASE + DICE_EAP_MIXER_SPACE_SIZE);
  if (!res) {
  ...
  res = device_.readReg(DICE_EAP_BASE + DICE_EAP_PEAK_SPACE_OFFSET);
  if (!res) {
  ...
  res = device_.readReg(DICE_EAP_BASE + DICE_EAP_PEAK_SPACE_SIZE);
  if (!res) {
  ...
  res = device_.readReg(DICE_EAP_BASE + DICE_EAP_NEW_ROUTING_SPACE_OFFSET);
  if (!res) {
  ...
  res = device_.readReg(DICE_EAP_BASE + DICE_EAP_NEW_ROUTING_SPACE_SIZE);
  if (!res) {
  ...
  res = device_.readReg(DICE_EAP_BASE + DICE_EAP_NEW_STREAM_CFG_SPACE_OFFSET);
  if (!res) {
  ...
  res = device_.readReg(DICE_EAP_BASE + DICE_EAP_NEW_STREAM_CFG_SPACE_SIZE);
  if (!res) {
  ...
  res = device_.readReg(DICE_EAP_BASE + DICE_EAP_CURR_CFG_SPACE_OFFSET);
  if (!res) {
  ...
  res = device_.readReg(DICE_EAP_BASE + DICE_EAP_CURR_CFG_SPACE_SIZE);
  if (!res) {
  ...
  res = device_.readReg(DICE_EAP_BASE + DICE_EAP_STAND_ALONE_CFG_SPACE_OFFSET);
  if (!res) {
  ...
  res = device_.readReg(DICE_EAP_BASE + DICE_EAP_STAND_ALONE_CFG_SPACE_SIZE);
  if (!res) {
  ...
  res = device_.readReg(DICE_EAP_BASE + DICE_EAP_APP_SPACE_OFFSET);
  if (!res) {
  ...
  res = device_.readReg(DICE_EAP_BASE + DICE_EAP_APP_SPACE_SIZE);
  if (!res) {
  ...
  spdlog::debug(" Capability        : offset=0x{:x} size={}", capabilityOffset_,
                capabilitySize_);
  spdlog::debug(" Command           : offset=0x{:x} size={}", cmdOffset_,
                cmdSize_);
  spdlog::debug(" Mixer             : offset=0x{:x} size={}", mixerOffset_,
                mixerSize_);
  spdlog::debug(" Peak              : offset=0x{:x} size={}", peakOffset_,
                peakSize_);
  spdlog::debug(" New Routing       : offset=0x{:x} size={}", newRoutingOffset_,
                newRoutingSize_);
  spdlog::debug(" New Stream Config : offset=0x{:x} size={}",
                newStreamCfgOffset_, newStreamCfgSize_);
  spdlog::debug(" Current Config    : offset=0x{:x} size={}", currCfgOffset_,
                currCfgSize_);
  spdlog::debug(" Standalone        : offset=0x{:x} size={}", standaloneOffset_,
                standaloneSize_);
  spdlog::debug(" Application       : offset=0x{:x} size={}", appOffset_,
                appSize_);

  // Read capability registers
  std::expected<uint32_t, IOKitError> routerCapRes = device_.readReg(
      DICE_EAP_BASE + capabilityOffset_ + DICE_EAP_CAPABILITY_ROUTER_OFFSET);
  if (!routerCapRes) {
  ...
  spdlog::debug("Router capabilities: exposed={}, readonly={}, flashstored={}, "
                "entries={}",
                routerExposed_, routerReadonly_, routerFlashstored_,
                routerNumEntries_);

  std::expected<uint32_t, IOKitError> mixerCapRes = device_.readReg(
      DICE_EAP_BASE + capabilityOffset_ + DICE_EAP_CAPABILITY_MIXER_OFFSET);
  if (!mixerCapRes) {
  ...
  std::expected<uint32_t, IOKitError> generalCapRes = device_.readReg(
      DICE_EAP_BASE + capabilityOffset_ + DICE_EAP_CAPABILITY_GENERAL_OFFSET);
  if (!generalCapRes) {
  ...
  generalSupportDynstream_ =
      (generalCap >> DICE_EAP_CAP_GENERAL_STRM_CFG_EN) & 0x01;
  ...
  generalStreamCfgStored_ =
      (generalCap >> DICE_EAP_CAP_GENERAL_STRM_CFG_FLS) & 0x01;
  generalChip_ = (generalCap >> DICE_EAP_CAP_GENERAL_CHIP) & 0xFFFF;

  spdlog::debug("General capabilities: dynamic stream={}, flash={}, peak={}, "
                "stream stored={}",
                generalSupportDynstream_, generalSupportFlash_,
                generalPeakEnabled_, generalStreamCfgStored_);
  ...
  if (mixerExposed_) {
    ...
    if (!mixerRes) {
      ...
      spdlog::info(
          "Successfully initialized mixer with {} inputs and {} outputs",
...
std::expected<void, IOKitError> DiceEAP::loadFlashConfig() {
  ...
  uint32_t cmd =
      DICE_EAP_CMD_OPCODE_LD_FLASH_CFG | DICE_EAP_CMD_OPCODE_FLAG_LD_EXECUTE;
...
std::expected<void, IOKitError> DiceEAP::storeFlashConfig() {
  ...
  // Simple implementation to start with
  uint32_t cmd =
      DICE_EAP_CMD_OPCODE_ST_FLASH_CFG | DICE_EAP_CMD_OPCODE_FLAG_LD_EXECUTE;
  return commandHelper(cmd);
}

DiceEAP::CommandStatus DiceEAP::operationBusy() {
  // Read command register to check if it's still busy
  std::expected<uint32_t, IOKitError> cmdRes = device_.readReg(
      DICE_EAP_BASE + cmdOffset_ + DICE_EAP_COMMAND_OPCODE_OFFSET);
  if (!cmdRes) {
  ...
  // Check status
  std::expected<uint32_t, IOKitError> retvalRes = device_.readReg(
      DICE_EAP_BASE + cmdOffset_ + DICE_EAP_COMMAND_RETVAL_OFFSET);
  if (!retvalRes) {
...
std::expected<void, IOKitError> DiceEAP::updateConfigurationCache() {
  ...
  if (currCfgRoutingLow_) {
    auto res = currCfgRoutingLow_->read(DICE_EAP_CURRCFG_LOW_ROUTER_OFFSET);
    if (!res) {
  ...
  if (currCfgRoutingMid_) {
    auto res = currCfgRoutingMid_->read(DICE_EAP_CURRCFG_MID_ROUTER_OFFSET);
    if (!res) {
  ...
  if (currCfgRoutingHigh_) {
    auto res = currCfgRoutingHigh_->read(DICE_EAP_CURRCFG_HIGH_ROUTER_OFFSET);
    if (!res) {
  ...
  if (currCfgStreamLow_) {
    auto res = currCfgStreamLow_->read(DICE_EAP_CURRCFG_LOW_STREAM_OFFSET);
    if (!res) {
  ...
  if (currCfgStreamMid_) {
    auto res = currCfgStreamMid_->read(DICE_EAP_CURRCFG_MID_STREAM_OFFSET);
    if (!res) {
  ...
  if (currCfgStreamHigh_) {
    auto res = currCfgStreamHigh_->read(DICE_EAP_CURRCFG_HIGH_STREAM_OFFSET);
    if (!res) {
...
std::expected<std::vector<std::string>, IOKitError>
DiceEAP::getTxNames(unsigned int index) {
...
std::expected<std::vector<std::string>, IOKitError>
DiceEAP::getRxNames(unsigned int index) {
  DiceStreamConfig *config = getActiveStreamConfig();
  if (!config) {
    spdlog::error("No active stream configuration");
    return std::unexpected(IOKitError::NoDevice);
...
std::expected<void, IOKitError> DiceEAP::readReg(RegBase base, uint32_t offset,
                                                 uint32_t *value) {
...
std::expected<void, IOKitError> DiceEAP::writeReg(RegBase base, uint32_t offset,
                                                  uint32_t value) {
  uint64_t addr = offsetGen(base, offset, 4);
  if (addr == 0) {
    spdlog::error("Invalid register address");
...
std::expected<void, IOKitError> DiceEAP::readRegBlock(RegBase base,
                                                      uint32_t offset,
                                                      uint32_t *data,
                                                      size_t size) {
  ...
  std::expected<std::vector<uint32_t>, IOKitError> res =
      device_.readRegBlock(addr, size);
  if (!res) {
    return std::unexpected(res.error());
  }
...
std::expected<void, IOKitError> DiceEAP::writeRegBlock(RegBase base,
                                                       uint32_t offset,
                                                       const uint32_t *data,
                                                       size_t size) {
  uint64_t addr = offsetGen(base, offset, size);
...
std::expected<void, IOKitError> DiceEAP::commandHelper(uint32_t cmd) {
  ...
  std::expected<void, IOKitError> res =
      writeReg(RegBase::Command, DICE_EAP_COMMAND_OPCODE_OFFSET, cmd);
  if (!res) {
...
int DiceEAP::getCurrentConfig() const { return device_.getCurrentConfig(); }

std::expected<std::vector<uint32_t>, IOKitError>
DiceEAP::readApplicationSpace(uint32_t offset, size_t size) {
  spdlog::debug("Reading application space at offset 0x{:x}, size {}", offset,
                size);
  ...
  if (offset + size > appSize_) {
    spdlog::error("Application space read out of bounds: offset=0x{:x}, "
                  "size={}, max=0x{:x}",
  ...
  auto result =
      device_.readRegBlock(DICE_EAP_BASE + appOffset_ + offset, numWords * 4);
  if (!result) {
...
uint64_t DiceEAP::offsetGen(RegBase base, uint32_t offset, size_t size) {
  ...
  if (offset + size > maxSize) {
    spdlog::error(
        "Register access out of bounds: offset=0x{:x}, size={}, maxSize=0x{:x}",
        offset, size, maxSize);
    return 0;
  }
...
std::expected<void, IOKitError> DiceEAP::addRoute(DICE::RouteSource srcId,
                                                  unsigned int srcBase,
                                                  DICE::RouteDestination dstId,
  ...
  return router->createRoute(static_cast<unsigned char>(srcId) + srcBase,
...
std::expected<void, IOKitError> DiceEAP::setupDefaultRouterConfigLow() {
  ...
  case DICE_EAP_CAP_GENERAL_CHIP_DICEMINI:
    ...
    if (generalChip_ == DICE_EAP_CAP_GENERAL_CHIP_DICEJR) {
      for (unsigned int i = 0; i < 8; i++) {
        addRoute(DICE::RouteSource::InS1, i, DICE::RouteDestination::ATX0,
                 i + 8);
      }
    }
    for (unsigned int i = 0; i < 8; i++) {
    ...
    for (unsigned int i = 0; i < 8; i++) {
      addRoute(DICE::RouteSource::ADAT, i, DICE::RouteDestination::Mixer0,
               i + 8);
    }
...
std::expected<void, IOKitError> DiceEAP::setupDefaultRouterConfigHigh() {
  ...
  // For most devices, high-rate config is the same as low-rate with some
...
std::string DiceEAP::getSourceName(DICE::RouteSource srcId,
...
std::string DiceEAP::getDestinationName(DICE::RouteDestination dstId,
                                        unsigned int channel) {
  char buffer[32];
