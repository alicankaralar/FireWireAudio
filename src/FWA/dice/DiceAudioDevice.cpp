...
{
    spdlog::info(
        "********** DICE INIT: Initializing DiceAudioDevice (GUID: "
        "0x%llx) **********",
        getGuid());
    ...
    spdlog::info("DICE INIT: Starting initIoFunctions() to set up register access");
    std::expected<void, IOKitError> ioFuncResult = initIoFunctions();
    if (!ioFuncResult)
    {
        spdlog::error("DICE INIT ERROR: Failed to initialize DICE I/O functions: {}", static_cast<int>(ioFuncResult.error()));
        return std::unexpected(ioFuncResult.error());
    }
    spdlog::info("DICE INIT: Successfully initialized I/O functions");

    // Detect DICE chip type
    spdlog::info("DICE INIT: Detecting DICE chip type via EAP capability register");
    std::expected<uint32_t, IOKitError> generalCapResult = readGlobalReg(DICE_EAP_CAPABILITY_GENERAL_OFFSET);
    if (generalCapResult)
    {
        ...
        spdlog::info("DICE INIT: Read capability register successfully: 0x{:08x}", generalCap);

        switch (static_cast<DICE::DiceChipType>(chipTypeValue))
        {
        case DICE::DiceChipType::DiceII:
            spdlog::info("DICE INIT: Detected DICE II chipset");
            break;
        case DICE::DiceChipType::DiceMini:
            spdlog::info("DICE INIT: Detected DICE Mini chipset");
            break;
        case DICE::DiceChipType::DiceJr:
            spdlog::info("DICE INIT: Detected DICE JR chipset");
            break;
        default:
            spdlog::info("DICE INIT: Unknown DICE chipset type: %d (0x{:08x})", chipTypeValue, generalCap);
            break;
        }
    }
    else
    {
        spdlog::warn(
            "DICE INIT WARNING: Could not determine DICE chip type - EAP "
            "capability read failed with error: {}",
            static_cast<int>(generalCapResult.error()));
    }
    ...
    if (DiceEAP::supportsEAP(*this))
    {
        ...
        if (!eapInit)
        {
            spdlog::warn("Could not initialize EAP interface: {}. EAP will be disabled.", static_cast<int>(eapInit.error()));
            eap_.reset();
            m_supportsEAP = false;
        }
        else
...
std::expected<void, IOKitError> DiceAudioDevice::discoverCapabilities()
{
    ...
    {
        spdlog::error("Failed to get supported sample rates: {}", static_cast<int>(supportedRatesResult.error()));
    ...
    if (!sampleRateResult)
    {
        spdlog::error("Failed to get current sample rate: {}", static_cast<int>(sampleRateResult.error()));
    }
    else
    {
...
bool DiceAudioDevice::isDiceJr() const { return chipType_ == static_cast<int>(DICE::DiceChipType::DiceJr); }
...
{
    std::expected<uint32_t, IOKitError> clockRegResult = readGlobalReg(DICE_REGISTER_GLOBAL_CLOCK_SELECT_OFFSET);
    if (!clockRegResult)
    {
...
std::expected<void, IOKitError> DiceAudioDevice::setSampleRate(int sampleRate)
{
    ...
    case 32000:
    {
        std::expected<bool, FWA::IOKitError> supportedResult
            = maskedCheckNotZeroGlobalReg(DICE_REGISTER_GLOBAL_CLOCKCAPABILITIES_OFFSET, DICE_CLOCKCAP_RATE_32K);
        if (!supportedResult)
        {
    ...
    {
        std::expected<bool, FWA::IOKitError> supportedResult
            = maskedCheckNotZeroGlobalReg(DICE_REGISTER_GLOBAL_CLOCKCAPABILITIES_OFFSET, DICE_CLOCKCAP_RATE_44K1);
        if (!supportedResult)
        {
    ...
    {
        std::expected<bool, FWA::IOKitError> supportedResult
            = maskedCheckNotZeroGlobalReg(DICE_REGISTER_GLOBAL_CLOCKCAPABILITIES_OFFSET, DICE_CLOCKCAP_RATE_48K);
        if (!supportedResult)
        {
    ...
    {
        std::expected<bool, FWA::IOKitError> supportedResult
            = maskedCheckNotZeroGlobalReg(DICE_REGISTER_GLOBAL_CLOCKCAPABILITIES_OFFSET, DICE_CLOCKCAP_RATE_88K2);
        if (!supportedResult)
        {
    ...
    {
        std::expected<bool, FWA::IOKitError> supportedResult
            = maskedCheckNotZeroGlobalReg(DICE_REGISTER_GLOBAL_CLOCKCAPABILITIES_OFFSET, DICE_CLOCKCAP_RATE_96K);
        if (!supportedResult)
        {
    ...
    {
        std::expected<bool, FWA::IOKitError> supportedResult
            = maskedCheckNotZeroGlobalReg(DICE_REGISTER_GLOBAL_CLOCKCAPABILITIES_OFFSET, DICE_CLOCKCAP_RATE_176K4);
        if (!supportedResult)
        {
    ...
    {
        std::expected<bool, FWA::IOKitError> supportedResult
            = maskedCheckNotZeroGlobalReg(DICE_REGISTER_GLOBAL_CLOCKCAPABILITIES_OFFSET, DICE_CLOCKCAP_RATE_192K);
        if (!supportedResult)
        {
    ...
    std::expected<void, IOKitError> writeResult = writeGlobalReg(DICE_REGISTER_GLOBAL_CLOCK_SELECT_OFFSET, clockReg);
    if (!writeResult)
    {
    ...
    auto verifyRegResult = readGlobalReg(DICE_REGISTER_GLOBAL_CLOCK_SELECT_OFFSET);
    if (!verifyRegResult)
    {
    ...
    while (((statusReg & DICE_STATUS_SOURCE_LOCKED) == 0 || DICE_GET_RATE(clockReg) != DICE_STATUS_GET_NOMINAL_RATE(statusReg))
           && attempts < 20)
    {
        usleep(100000); // 100ms
...
std::expected<std::vector<int>, IOKitError> DiceAudioDevice::getSupportedSampleRates()
{
    ...
    std::expected<bool, IOKitError> check32k
        = maskedCheckNotZeroGlobalReg(DICE_REGISTER_GLOBAL_CLOCKCAPABILITIES_OFFSET, DICE_CLOCKCAP_RATE_32K);
    if (!check32k)
    ...
    std::expected<bool, IOKitError> check44k1
        = maskedCheckNotZeroGlobalReg(DICE_REGISTER_GLOBAL_CLOCKCAPABILITIES_OFFSET, DICE_CLOCKCAP_RATE_44K1);
    if (!check44k1)
    ...
    std::expected<bool, IOKitError> check48k
        = maskedCheckNotZeroGlobalReg(DICE_REGISTER_GLOBAL_CLOCKCAPABILITIES_OFFSET, DICE_CLOCKCAP_RATE_48K);
    if (!check48k)
    ...
    std::expected<bool, IOKitError> check88k2
        = maskedCheckNotZeroGlobalReg(DICE_REGISTER_GLOBAL_CLOCKCAPABILITIES_OFFSET, DICE_CLOCKCAP_RATE_88K2);
    if (!check88k2)
    ...
    std::expected<bool, IOKitError> check96k
        = maskedCheckNotZeroGlobalReg(DICE_REGISTER_GLOBAL_CLOCKCAPABILITIES_OFFSET, DICE_CLOCKCAP_RATE_96K);
    if (!check96k)
    ...
    std::expected<bool, IOKitError> check176k4
        = maskedCheckNotZeroGlobalReg(DICE_REGISTER_GLOBAL_CLOCKCAPABILITIES_OFFSET, DICE_CLOCKCAP_RATE_176K4);
    if (!check176k4)
    ...
    std::expected<bool, IOKitError> check192k
        = maskedCheckNotZeroGlobalReg(DICE_REGISTER_GLOBAL_CLOCKCAPABILITIES_OFFSET, DICE_CLOCKCAP_RATE_192K);
    if (!check192k)
...
std::expected<std::string, IOKitError> DiceAudioDevice::getNickname()
{
    std::expected<std::vector<uint32_t>, IOKitError> nameStringResult
        = readGlobalRegBlock(DICE_REGISTER_GLOBAL_NICK_NAME_OFFSET, DICE_NICK_NAME_SIZE);
    if (!nameStringResult)
    {
...
{
    // Create a vector of uint32_t with the right size (in bytes) to hold the
    // string data
    ...
    std::expected<void, IOKitError> writeResult
        = writeGlobalRegBlock(DICE_REGISTER_GLOBAL_NICK_NAME_OFFSET, nameData.data(), DICE_NICK_NAME_SIZE);
    if (!writeResult)
    {
...
{
    std::expected<void, IOKitError> writeResult = writeGlobalReg(DICE_REGISTER_GLOBAL_ENABLE_OFFSET, DICE_ISOSTREAMING_ENABLE);
    if (!writeResult)
    {
...
{
    std::expected<void, IOKitError> writeResult = writeGlobalReg(DICE_REGISTER_GLOBAL_ENABLE_OFFSET, DICE_ISOSTREAMING_DISABLE);
    if (!writeResult)
    {
...
{
    std::expected<uint32_t, IOKitError> enableRegResult = readGlobalReg(DICE_REGISTER_GLOBAL_ENABLE_OFFSET);
    if (!enableRegResult)
    {
...
uint64_t DiceAudioDevice::txOffsetGen(unsigned int i, uint32_t offset, size_t length)
{
    ...
    {
        spdlog::error(
            "TX Register offset+length out of bounds: base=0x{:X}, "
            "offset=0x{:X}, length={}, total_size=0x{:X}",
            m_tx_reg_offset,
            offset_tx,
            length,
            m_tx_reg_size);
...
uint64_t DiceAudioDevice::rxOffsetGen(unsigned int i, uint32_t offset, size_t length)
{
    ...
    {
        spdlog::error(
            "RX Register offset+length out of bounds: base=0x{:X}, "
            "offset=0x{:X}, length={}, total_size=0x{:X}",
            m_rx_reg_offset,
            offset_rx,
            length,
...
{
    spdlog::info(
        "********** DICE IO INIT: Reading DICE register offsets and "
        "sizes **********");

    // Read offsets and sizes (returned in quadlets, but we use byte values)
    spdlog::info(
        "DICE IO INIT: Reading GLOBAL_PAR_SPACE_OFFSET register at "
        "offset 0x{:08x}",
        DICE_REGISTER_GLOBAL_PARAMETER_SPACE_OFFSET);
    std::expected<uint32_t, IOKitError> global_offset_res = readReg(DICE_REGISTER_GLOBAL_PARAMETER_SPACE_OFFSET);
    if (!global_offset_res)
    {
        spdlog::error(
            "DICE IO INIT ERROR: Could not initialize "
            "m_global_reg_offset - error code: {}",
            static_cast<int>(global_offset_res.error()));
    ...
    spdlog::info(
        ...
        "(byte value: 0x{:08x})",
        global_offset_res.value(),
        m_global_reg_offset);

    spdlog::info("DICE IO INIT: Reading GLOBAL_PAR_SPACE_SIZE register at offset 0x{:08x}", DICE_REGISTER_GLOBAL_PARAMETER_SPACE_SIZE);
    auto global_size_res = readReg(DICE_REGISTER_GLOBAL_PARAMETER_SPACE_SIZE);
    if (!global_size_res)
    {
        spdlog::error(
            "DICE IO INIT ERROR: Could not initialize m_global_reg_size "
            "- error code: {}",
            static_cast<int>(global_size_res.error()));
    ...
    spdlog::info(
        ...
        "(byte value: 0x{:08x})",
        global_size_res.value(),
        m_global_reg_size);

    spdlog::info("DICE IO INIT: Reading TX_PAR_SPACE_OFFSET register at offset 0x{:08x}", DICE_REGISTER_TX_PARAMETER_SPACE_OFFSET);
    std::expected<uint32_t, IOKitError> tx_offset_res = readReg(DICE_REGISTER_TX_PARAMETER_SPACE_OFFSET);
    if (!tx_offset_res)
    {
        spdlog::error(
            "DICE IO INIT ERROR: Could not initialize m_tx_reg_offset - "
            "error code: {}",
            static_cast<int>(tx_offset_res.error()));
        return std::unexpected(tx_offset_res.error());
    }
    m_tx_reg_offset = tx_offset_res.value() * 4;
    spdlog::info(
        "DICE IO INIT: Successfully read TX_PAR_SPACE_OFFSET: 0x{:08x} "
        "(byte value: 0x{:08x})",
        tx_offset_res.value(),
        m_tx_reg_offset);
    ...
    auto tx_size_param_res = readReg(m_tx_reg_offset + DICE_REGISTER_TX_SIZE_TX_OFFSET);
    ...
    auto rx_size_param_res = readReg(m_rx_reg_offset + DICE_REGISTER_RX_SIZE_RX_OFFSET);
...
std::expected<uint32_t, IOKitError> DiceAudioDevice::readReg(uint64_t offset, uint64_t base)
{
    spdlog::debug(
        "DiceAudioDevice::readReg - Reading from offset 0x{:x} "
        "[hi:0x{:04x}, lo:0x{:08x}]",
        offset,
        static_cast<UInt16>((offset >> 32) & 0xFFFF),
        static_cast<UInt32>(offset & 0xFFFFFFFF));

    if (!deviceInterface)
    {
        spdlog::error(
            "DiceAudioDevice::readReg - deviceInterface is null. Cannot "
            "read from offset 0x{:x}",
            offset);
    ...
    UInt32 generation;
    spdlog::debug("DiceAudioDevice::readReg - Getting bus generation...");
    IOReturn status = (*deviceInterface)->GetBusGeneration(deviceInterface, &generation);
    if (status != kIOReturnSuccess)
    {
        spdlog::error(
            "DiceAudioDevice::readReg - Failed to get bus generation: "
            "0x{:x} for offset 0x{:x}",
            status,
            offset);
        return std::unexpected(static_cast<IOKitError>(status));
    }
    spdlog::debug("DiceAudioDevice::readReg - Got bus generation: {}", generation);

    spdlog::debug(
        "DiceAudioDevice::readReg - Calling ReadQuadlet for offset "
        "0x{:x} [hi:0x{:04x}, lo:0x{:08x}]",
        offset,
        addr.addressHi,
        addr.addressLo);
    ...
    {
        spdlog::error(
            "DiceAudioDevice::readReg - ReadQuadlet FAILED for offset "
            "0x{:x} with status 0x{:x}",
            offset,
            status); // Removed IOKitErrorToString call
        // Map IOReturn to IOKitError
        return std::unexpected(static_cast<IOKitError>(status));
    }

    // DICE registers are little-endian, so we need to swap bytes on read
    UInt32 swappedResult = CFSwapInt32LittleToHost(result);
    spdlog::debug(
        "DiceAudioDevice::readReg - Successfully read 0x{:08x} from "
        "offset 0x{:x} (byte-swapped: 0x{:08x})",
        result,
        offset,
        swappedResult);
...
std::expected<void, IOKitError> DiceAudioDevice::writeReg(uint64_t offset, uint32_t data)
{
    ...
    status = ((IOFireWireDeviceInterface_t *)deviceInterface)
                 ->WriteQuadlet(deviceInterface, 0, &addr, dataToWrite, kFWFailOnReset, generation);

    if (status != kIOReturnSuccess)
    {
        spdlog::error(
            "DiceAudioDevice::writeReg - WriteQuadlet failed for offset "
            "0x{:x} with status 0x{:x}",
            offset,
            status);
        // Attempt to map IOReturn to IOKitError, default to general Error if not
        // found
        return std::unexpected(static_cast<IOKitError>(status));
    }
    return {};
...
std::expected<std::vector<uint32_t>, IOKitError> DiceAudioDevice::readRegBlock(uint64_t offset, size_t length)
{
    ...
    for (size_t i = 0; i < numQuadlets; ++i)
    {
        ...
        {
            spdlog::error(
                "DiceAudioDevice::readRegBlock - Failed reading quadlet {} "
                "at offset 0x{:x}",
                i,
                offset + (i * 4));
            return std::unexpected(result.error());
...
std::expected<void, IOKitError> DiceAudioDevice::writeRegBlock(uint64_t offset, const uint32_t *data, size_t length)
{
    ...
    size_t numQuadlets = length / 4;
    // DICE registers are little-endian, so create a temporary buffer with
    // byte-swapped data
    ...
    for (size_t i = 0; i < numQuadlets; ++i)
    {
        ...
        {
            spdlog::error(
                "DiceAudioDevice::writeRegBlock - Failed writing quadlet "
                "{} at offset 0x{:x}",
                i,
                offset + (i * 4));
            return std::unexpected(result.error());
...
{
    // Placeholder implementation - splits by null terminator found within the
    // string
...
std::vector<std::string> DiceAudioDevice::getTxNameString(unsigned int i)
{
    ...
    std::expected<std::vector<uint32_t>, IOKitError> readResult = readTxRegBlock(i, DICE_REGISTER_TX_NAMES_BASE_OFFSET, DICE_TX_NAMES_SIZE);
    if (!readResult)
    {
...
std::vector<std::string> DiceAudioDevice::getRxNameString(unsigned int i)
{
    ...
    std::expected<std::vector<uint32_t>, IOKitError> readResult = readRxRegBlock(i, DICE_REGISTER_RX_NAMES_BASE_OFFSET, DICE_RX_NAMES_SIZE);
    if (!readResult)
    {
...
std::vector<std::string> DiceAudioDevice::getClockSourceNameString()
{
    ...
    std::expected<std::vector<uint32_t>, IOKitError> readResult
        = readGlobalRegBlock(DICE_REGISTER_GLOBAL_CLOCKSOURCENAMES_OFFSET, DICE_CLOCKSOURCENAMES_SIZE);
    if (!readResult)
    {
...
// Helper function to convert IOKitError to string for more descriptive logging
std::string IOKitErrorToString(IOKitError error)
{
...
std::expected<uint32_t, IOKitError> DiceAudioDevice::readEAPReg(DICE::EAPSection section, uint32_t offset)
{
    spdlog::debug("Reading EAP register - Section: {}, Offset: 0x{:x}", getEAPSectionInfo(section)->name, offset);

    uint64_t addr = eapOffsetGen(section, offset, 4);
    if (addr == DICE_INVALID_OFFSET)
    {
        spdlog::error("Invalid EAP address generated for section {} at offset 0x{:x}", getEAPSectionInfo(section)->name, offset);
        return std::unexpected(IOKitError::BadArgument);
    }
    ...
    {
        spdlog::debug(
            "Successfully read EAP register - Section: {}, Offset: "
            "0x{:x}, Value: 0x{:08x}",
            getEAPSectionInfo(section)->name,
            offset,
            result.value());
...
std::expected<void, IOKitError> DiceAudioDevice::writeEAPReg(DICE::EAPSection section, uint32_t offset, uint32_t data)
{
    if (isEAPSectionReadOnly(section))
    {
        spdlog::error("Attempted to write to read-only EAP section: {}", getEAPSectionInfo(section)->name);
        return std::unexpected(IOKitError::NotWritable);
    }

    spdlog::debug("Writing EAP register - Section: {}, Offset: 0x{:x}, Data: 0x{:08x}", getEAPSectionInfo(section)->name, offset, data);
    ...
    {
        spdlog::debug("Successfully wrote EAP register - Section: {}, Offset: 0x{:x}", getEAPSectionInfo(section)->name, offset);
    }
    return result;
}

std::expected<std::vector<uint32_t>, IOKitError> DiceAudioDevice::readEAPBlock(DICE::EAPSection section, uint32_t offset, size_t length)
{
    spdlog::debug("Reading EAP block - Section: {}, Offset: 0x{:x}, Length: {}", getEAPSectionInfo(section)->name, offset, length);
...
DiceAudioDevice::writeEAPBlock(DICE::EAPSection section, uint32_t offset, const uint32_t *data, size_t length)
{
    if (isEAPSectionReadOnly(section))
    {
        spdlog::error("Attempted to write to read-only EAP section: {}", getEAPSectionInfo(section)->name);
    ...
    spdlog::debug("Writing EAP block - Section: {}, Offset: 0x{:x}, Length: {}", getEAPSectionInfo(section)->name, offset, length);
    ...
    {
        spdlog::debug("Successfully wrote EAP block - Section: {}, Offset: 0x{:x}", getEAPSectionInfo(section)->name, offset);
    }
    return result;
}

// EAP helper methods
uint64_t DiceAudioDevice::eapOffsetGen(DICE::EAPSection section, uint32_t offset, size_t length)
{
    ...
    {
        spdlog::error(
            "EAP access exceeds section bounds - Section: {}, Offset: "
...
const DICE::EAPSectionInfo *DiceAudioDevice::getEAPSectionInfo(DICE::EAPSection section) const
{
