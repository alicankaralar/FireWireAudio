...
std::expected<void, IOKitError> AudioDevice::init()
{
    ...
    if (!fwDevice_)
    { // Check fwDevice_ passed from constructor
        spdlog::error(
            "AudioDevice::init: Cannot initialize without a valid "
            "fwDevice service.");
        return std::unexpected(IOKitError(kIOReturnNotAttached));
    }

    // Get bus controller directly from fwDevice_
    IOReturn result = IORegistryEntryGetParentEntry(fwDevice_, kIOServicePlane, &busController_);
    if (result != kIOReturnSuccess || !busController_)
    {
        spdlog::error("AudioDevice::init: Failed to get busController from fwDevice_: 0x{:x}", result);
    ...
    {
        spdlog::warn(
            "AudioDevice::init: Failed to read vendor/model info: 0x{:x}. "
    ...
    {
        spdlog::info(
            "AudioDevice::init: Read VendorID=0x{:08X}, ModelID=0x{:08X}, "
            "VendorName='{}'",
            vendorID_,
            modelID_,
            vendorName_);
    ...
    result = IORegistryEntryGetChildIterator(fwDevice_, kIOServicePlane, &childIterator);
    if (result == kIOReturnSuccess && childIterator != 0)
    {
        ...
        while ((childService = IOIteratorNext(childIterator)) != 0)
        {
            ...
            {
                spdlog::debug("AudioDevice::init: Found IOFireWireAVCUnit child service: {:#x}", childService);
    ...
    if (avcUnit_ == 0)
    {
        ...
        {
            spdlog::debug(
                "AudioDevice::init: fwDevice_ ({:#x}) itself conforms to "
                "IOFireWireAVCUnit.",
                fwDevice_);
        ...
        {
            spdlog::info(
                "AudioDevice::init: No AVC unit found for device. "
    ...
    */
    ...
    try
    {
        if (hasAvcCapability_)
        {
            ...
            {
                spdlog::error(
                    "AudioDevice::init: Failed to activate CommandInterface "
                    "before parsing: 0x{:x}",
            ...
            {
                spdlog::debug(
                    "AudioDevice::init: CommandInterface activated (or was "
                    "already active).");
            }
        }
        else
        {
            spdlog::info(
                "AudioDevice::init: Skipping CommandInterface creation for "
                "non-AVC device.");
        }
    }
    ...
    {
        spdlog::critical(
            "AudioDevice::init: Failed to create CommandInterface. "
        ...
        hasAvcCapability_ = false;
        spdlog::warn(
            "AudioDevice::init: Continuing without AVC functionality due "
            "to error.");
    }
    ...
    if (hasAvcCapability_ && commandInterface_)
    {
        ...
        {
            spdlog::info(
                "AudioDevice::init: Device capability parsing completed "
                "successfully.");
        }
    }
    else
...
IOReturn AudioDevice::createFWDeviceInterface()
{
    ...
    if (!result)
    {
        ...
        comErr = (*theCFPlugInInterface)
                     ->QueryInterface(theCFPlugInInterface, CFUUIDGetUUIDBytes(kIOFireWireNubInterfaceID), (void **)&deviceInterface);
        if (comErr == S_OK)
        {
        ...
        if (!result)
        {
            ...
            // device interface
            if (avcInterface_)
                result = (*deviceInterface)->OpenWithSessionRef(deviceInterface, (*avcInterface_)->getSessionRef(avcInterface_));
            else
...
{
    if (!fwDevice_)
    { // Use fwDevice_ instead of fwUnit_
        spdlog::error("AudioDevice::readVendorAndModelInfo: fwDevice_ is null.");
        return kIOReturnNotReady;
    }
    CFMutableDictionaryRef properties = nullptr;
    IOReturn result = IORegistryEntryCreateCFProperties(fwDevice_, &properties, kCFAllocatorDefault,
                                                        kNilOptions); // Use fwDevice_
    if (result != kIOReturnSuccess)
    {
        spdlog::error(
            "AudioDevice::readVendorAndModelInfo: "
    ...
    if (unitVendorIDRef)
    {
        if (CFGetTypeID(unitVendorIDRef) == CFNumberGetTypeID())
        {
            ...
            {
                spdlog::warn(
                    "AudioDevice::readVendorAndModelInfo: CFNumberGetValue "
        ...
        {
            spdlog::warn(
                "AudioDevice::readVendorAndModelInfo: Vendor_ID property is "
                "not a CFNumber.");
        }
    }
    else
    ...
    if (unitModelIDRef)
    {
        ...
        {
            spdlog::warn(
                "AudioDevice::readVendorAndModelInfo: Model_ID property is "
                "not a CFNumber.");
        }
    }
    else
    ...
    if (unitVendorStrDesc)
    {
        if (CFGetTypeID(unitVendorStrDesc) == CFStringGetTypeID())
        {
            ...
            {
                spdlog::warn(
                    "AudioDevice::readVendorAndModelInfo: cfStringToString "
        ...
        {
            spdlog::warn(
                "AudioDevice::readVendorAndModelInfo: FireWire Vendor Name "
...
void AudioDevice::detectDICESupport()
{
    ...
    {
        spdlog::info(
            "AudioDevice::detectDICESupport: Found TCAT vendor ID "
    ...
    {
        CFMutableDictionaryRef properties = nullptr;
        IOReturn result = IORegistryEntryCreateCFProperties(fwDevice_, &properties, kCFAllocatorDefault, kNilOptions);
        ...
        if (result == kIOReturnSuccess && properties)
        {
            ...
            CFStringRef modelName = (CFStringRef)CFDictionaryGetValue(properties, CFSTR("Model"));

            if (modelName && CFGetTypeID(modelName) == CFStringGetTypeID())
            {
                std::string model = Helpers::cfStringToString(modelName);
                if (model.find("DICE") != std::string::npos || model.find("dice") != std::string::npos)
                {
...
AudioDevice::checkControlResponse(const std::expected<std::vector<uint8_t>, IOKitError>& result, const char *commandName)
{
    if (!result)
    {
...
std::expected<void, IOKitError>
AudioDevice::checkDestPlugConfigureControlSubcommandResponse(const std::vector<uint8_t>& response, const char *commandName)
{
    if (response.size() < 13)
    {
    ...
    {
        spdlog::error(
            "{} failed: No connection (Unknown subfunction) reported in "
            "subcommand status.",
            commandName);
        return std::unexpected(IOKitError::BadArgument);
    }
...
std::vector<uint8_t>
AudioDevice::buildSetStreamFormatControlCmd(PlugDirection direction, uint8_t plugNum, const std::vector<uint8_t>& formatBytes)
{
    std::vector<uint8_t> cmd;
    ...
    {
        spdlog::error(
            "buildSetStreamFormatControlCmd: Can only set format for Iso "
            "plugs (num < 128).");
        return {};
    }
    ...
    uint8_t streamPosition1)
{
    ...
    if (!info_.hasMusicSubunit())
        return std::unexpected(IOKitError::NotFound);
    spdlog::info(
        "AudioDevice::connectMusicPlug: Type=0x{:02x}, ID={}, "
        "DestPlug={}, StreamPos=[{}, {}]",
        musicPlugType,
        musicPlugID,
        destSubunitPlugID,
...
std::expected<void, IOKitError> AudioDevice::disconnectMusicPlug(uint8_t musicPlugType, uint16_t musicPlugID)
{
    if (!hasAvcCapability_)
        return std::unexpected(IOKitError::Unsupported);
    if (!commandInterface_)
    ...
    std::vector<uint8_t> cmd
        = buildDestPlugConfigureControlCmd(kAVCDestPlugSubfuncDisconnect, musicPlugType, musicPlugID, 0xFF, 0xFF, 0xFF);
...
std::expected<void, IOKitError>
AudioDevice::setUnitIsochPlugStreamFormat(PlugDirection direction, uint8_t plugNum, const AudioStreamFormat& format)
{
    if (!hasAvcCapability_)
        return std::unexpected(IOKitError::Unsupported);
    if (!commandInterface_)
    ...
    spdlog::info(
        "AudioDevice::setUnitIsochPlugStreamFormat: Dir={}, PlugNum={}, "
        "Format=[{}]",
    ...
    {
        spdlog::error(
            "setUnitIsochPlugStreamFormat: Failed to serialize the "
    ...
    uint8_t newStreamPosition1)
{
    ...
    spdlog::info(
        "AudioDevice::changeMusicPlugConnection: Type=0x{:02x}, ID={}, "
        "NewDestPlug={}, NewStreamPos=[{}, {}]",
        musicPlugType,
        musicPlugID,
        newDestSubunitPlugID,
        newStreamPosition0,
        newStreamPosition1);
...
std::expected<void, IOKitError> AudioDevice::defaultConfigureMusicPlugs()
{
    ...
    spdlog::info(
        "AudioDevice::defaultConfigureMusicPlugs: Resetting connections "
        "to default.");
...
{
    spdlog::info(
        "AudioDevice::initializeBasicCapabilities: Setting up basic "
    ...
    if (deviceInterface)
    {
        ...
        if (result == kIOReturnSuccess && properties)
        {
            ...
            CFBooleanRef isoCapable = (CFBooleanRef)CFDictionaryGetValue(properties, CFSTR("IOFireWireIsochronousCapabilities"));

            if (isoCapable && CFGetTypeID(isoCapable) == CFBooleanGetTypeID())
            {
                hasIsochronousCapability = CFBooleanGetValue(isoCapable);
                spdlog::info(
                    "AudioDevice::initializeBasicCapabilities: Device has "
    ...
    spdlog::info(
        "AudioDevice::initializeBasicCapabilities: Device will have "
        "limited functionality");
    spdlog::info(
        "AudioDevice::initializeBasicCapabilities: GUID: 0x{:x}, "
        "Vendor: {}, Model: {}",
        guid_,
        vendorName_,
        deviceName_);
