...
void readAudioMixerRegisters(IOFireWireDeviceInterface **deviceInterface,
  ...
  // Read MIXER_NUMOFCH
  UInt64 numOfChAddr = AUDIO_MIXER_NUMOFCH_ADDR; // Use absolute address from
  ...
  if (status == kIOReturnSuccess) {
    ...
    UInt32 hostValue = CFSwapInt32LittleToHost(numOfChValue);
    device.mixerRxChannels = static_cast<UInt8>(hostValue & 0xFF);
