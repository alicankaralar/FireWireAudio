 ...
 *
 * Reads the EAP capability space offset, then reads General, Router, and Mixer
 * capability registers. Updates device.diceChipType based on the General
 * capability register. Stores raw capability register values in
 * device.diceRegisters. Assumes the DICE base address for reading the EAP
 * offset is DICE_REGISTER_BASE.
 *
 ...
 * @return true if EAP capability space offset and General capability were read
 ...
 *
 * Reads the EAP current configuration space offset, then reads TX/RX stream
 * counts from the appropriate configuration block (currently hardcoded to LOW).
 * Updates device.txStreamCount and device.rxStreamCount.
 * Stores raw count register values in device.diceRegisters.
 * Assumes the DICE base address for reading the EAP offset is
 * DICE_REGISTER_BASE.
 *
 * @param deviceInterface Pointer-to-pointer to an opened
 * IOFireWireDeviceInterface.
 ...
 * @param device Reference to the FireWireDevice struct to populate.
 * @param generation The current bus generation number.
 * @return true if EAP current config space offset and TX count were read
 * successfully, false otherwise.
 */
