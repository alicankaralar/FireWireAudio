...
namespace FWA::SCANNER
{
	...
	void exploreDiceMemoryLayout(IOFireWireDeviceInterface **deviceInterface, io_service_t service,
															 FireWireDevice &device, UInt32 generation, uint64_t baseAddr)
	{
		std::cerr << "Debug [Utils]: Exploring memory around address 0x"
		...
		if (baseAddr == 0xffffe0000000ULL)
		{
					...
					channelNamesAddr <= baseAddr + EXPLORE_RANGE * 4)
			{
				...
				std::cerr << "Debug [Utils]: Extended scan range to " << EXPLORE_RANGE * 16
									<< " bytes to capture more channel names" << std::endl;
			}
		}

		std::cerr << "Debug [Utils]: Scanning " << EXPLORE_RANGE * 16 << " bytes below and above 0x"
							<< std::hex << baseAddr << std::dec << " for possible DICE structures" << std::endl;

		// First look for a valid version register
		std::cerr << "\n--- Potential Version Registers ---" << std::endl;
		for (int offset = -EXPLORE_RANGE; offset <= EXPLORE_RANGE; offset += EXPLORE_STEP)
		{
			uint64_t testAddr = baseAddr + offset * 4; // 4 bytes per quadlet
			UInt32 value = 0;

			IOReturn status = FWA::SCANNER::safeReadQuadlet(deviceInterface, service, testAddr, value, generation);
			if (status == kIOReturnSuccess)
			{
				// Check if this could be a version register (typically in format A.B.C.D where each is a byte)
				// A valid version is usually something like 1.0.7.0 or similar
				uint32_t swappedValue = CFSwapInt32LittleToHost(value); // DICE uses Little Endian
				...
				bool isLikelyVersion = versionA < 10 && versionB < 20 && versionC < 50 && versionD < 10;

				if (isLikelyVersion)
				{
					std::cerr << "Addr: 0x" << std::hex << testAddr << " Value: 0x" << swappedValue << std::dec;
					std::cerr << " **POSSIBLE VERSION: " << (int)versionA << "." << (int)versionB << "."
										<< (int)versionC << "." << (int)versionD << "**";
					if (!ascii.empty())
						std::cerr << " (ASCII: '" << ascii << "')";
					std::cerr << std::endl;
				}
				else if (!ascii.empty() && ascii.length() >= 2)
				{ // Log potentially interesting ASCII too
					// std::cerr << "Addr: 0x" << std::hex << testAddr << " Value: 0x" << swappedValue << std::dec;
		...
		for (int offset = -EXPLORE_RANGE; offset <= EXPLORE_RANGE; offset += EXPLORE_STEP)
		{
				...
				else
				{
					if (inString)
					{
						...
						{ // Arbitrary length threshold
							std::cerr << "Found string at 0x" << std::hex << stringStartAddr << ": '" << currentString << "'" << std::dec << std::endl;
						}
						currentString.clear();
						inString = false;
					}
			...
			{
				if (inString)
				{ // End of string due to read error
					if (currentString.length() > 4)
					{
						std::cerr << "Found string at 0x" << std::hex << stringStartAddr << ": '" << currentString << "'" << std::dec << std::endl;
					}
		...
		for (int offset = -EXPLORE_RANGE; offset <= EXPLORE_RANGE; offset += EXPLORE_STEP)
		{
			...
			{
				if (value == 0)
				{ // Check raw Big Endian value for zero
					if (zeroCount == 0)
					{
				...
				{
					if (zeroCount > 4)
					{ // Report if more than 4 consecutive zeros
						std::cerr << "Found " << zeroCount << " consecutive zeros starting at 0x" << std::hex << zeroStartAddr << std::dec << std::endl;
					}
					zeroCount = 0;
				}
			}
	...
	void exploreTargetedMemoryAreas(IOFireWireDeviceInterface **deviceInterface, io_service_t service,
																	FireWireDevice &device, UInt32 generation, uint64_t discoveredDiceBase)
	{
		std::cerr << "Debug [Utils]: Exploring targeted memory areas..." << std::endl;

		// Explore key functional blocks based on datasheet memory map
		// Using discoveredDiceBase if available, otherwise falling back to hardcoded DICE_REGISTER_BASE
		uint64_t baseToUse = (discoveredDiceBase != DICE_INVALID_OFFSET) ? discoveredDiceBase : DICE_REGISTER_BASE;

		std::cerr << "\n--- Exploring GPCSR Area (0xC700_0000) ---" << std::endl;
		exploreDiceMemoryLayout(deviceInterface, service, device, generation, DICE_REGISTER_GPCSR_BASE);

		std::cerr << "\n--- Exploring Clock Controller Area (0xCE01_0000) ---" << std::endl;
		exploreDiceMemoryLayout(deviceInterface, service, device, generation, DICE_REGISTER_CLOCK_CONTROLLER_BASE);

		std::cerr << "\n--- Exploring AES Receiver Area (0xCE02_0000) ---" << std::endl;
		exploreDiceMemoryLayout(deviceInterface, service, device, generation, DICE_REGISTER_AES_RECEIVER_BASE);

		std::cerr << "\n--- Exploring Audio Mixer Area (0xCE06_0000) ---" << std::endl;
		exploreDiceMemoryLayout(deviceInterface, service, device, generation, DICE_REGISTER_AUDIO_MIXER_BASE);

		std::cerr << "\n--- Exploring AVS Sub System Area (0xCF00_0000) ---" << std::endl;
		exploreDiceMemoryLayout(deviceInterface, service, device, generation, DICE_REGISTER_AVS_SUB_SYSTEM_BASE);

		// Add other targeted areas as needed based on further analysis (e.g., InS, ADAT, ARM Audio Transceiver)

		std::cerr << "\nDebug [Utils]: Targeted memory exploration complete." << std::endl;
	}
	...
	void extractCoherentRegisterStrings(const FireWireDevice &device)
	{
		...
		for (const auto &addrPair : knownStringAddresses)
		{
			...
			if (tempRegisters.find(addr) == tempRegisters.end())
			{
				...
				std::cout << "  Checking " << description << " at 0x" << std::hex << addr << std::dec << "..." << std::endl;
				...
				{
					if (std::abs(static_cast<int64_t>(reg.first) - static_cast<int64_t>(addr)) < 64)
					{
						std::cout << "    Found nearby register at 0x" << std::hex << reg.first << std::dec << std::endl;
						foundNearby = true;
					}
				}
				...
				if (!foundNearby)
				{
					std::cout << "    No nearby registers found in current scan" << std::endl;
				}
			}
		}
		std::cout << std::endl;
		...
		for (const auto &range : addressRanges)
		{
			...
			std::cout << "Address Range: 0x" << std::hex << baseAddr << " - 0x"
								<< registers.back().first << std::dec << " ("
								<< registers.size() << " registers)" << std::endl;
			...
			for (const auto &reg : registers)
			{
				...
				uint32_t hostValue = CFSwapInt32LittleToHost(rawValue); // Convert to host endianness
				...
				if (swappedAscii.length() > ascii.length() ||
						(swappedAscii.length() == ascii.length() && swappedAscii.find_first_not_of("abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789") > ascii.find_first_not_of("abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789")))
				{
					...
					if (inString)
					{
						...
						{ // Minimum meaningful length
							std::cout << "  String at 0x" << std::hex << stringStartAddr
												<< std::dec << ": \"" << currentString << "\"" << std::endl;
						}
						currentString.clear();
						inString = false;
					}
		...
		for (const auto &regPair : device.diceRegisters)
		{
			...
			{
				std::string nameType = isNickNameBase ? "Device Nick Name" : (isTxNameBase ? "TX Channel Name" : "RX Channel Name");

				// The value is likely a pointer to the actual name data
				uint64_t nameDataAddr = baseAddr + (hostValue * 4); // Convert quadlet offset to byte address
		...
		for (const auto &regPair : device.diceRegisters)
		{
			...
			if (offset == DICE_REGISTER_GLOBAL_CLOCKSOURCENAMES)
			{
				...
				std::cout << "Clock Source Names Base at 0x" << std::hex << addr
									<< " points to 0x" << clockSourceNamesAddr << std::dec << std::endl;
				break;
			}
		}
		...
		if (clockSourceNamesAddr != 0)
		{
			...
			for (int i = 0; i < 16; i++)
			{ // Assume up to 16 clock sources
				std::string clockName;
				bool foundClockName = false;
				uint64_t clockNameAddr = clockSourceNamesAddr + (i * 16); // Typically 16 bytes per name

				for (int j = 0; j < 4; j++)
				{ // Up to 4 quadlets per name
				...
				if (foundClockName)
				{
					std::cout << "  Clock Source " << i << " at 0x" << std::hex << clockNameAddr
										<< std::dec << ": \"" << clockName << "\"" << std::endl;
				}
			}
		}
		else
