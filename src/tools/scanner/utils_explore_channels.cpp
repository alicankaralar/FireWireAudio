...
void exploreChannelNamesArea(IOFireWireDeviceInterface **deviceInterface,
  ...
  // We'll scan 1024 bytes (256 quadlets) before and 4096 bytes (1024 quadlets)
  // after
  const int PRE_RANGE = 256;   // Quadlets before
  const int POST_RANGE = 1024; // Quadlets after
  const int STEP = 1;          // Scan every quadlet

  std::cout << "Scanning " << PRE_RANGE * 4 << " bytes before and "
            << POST_RANGE * 4 << " bytes after 0x" << std::hex
  ...
  // Check if we have a pattern of "OUTPUT CH1", "OUTPUT CH2", etc. or "INPUT
  // CH1", "INPUT CH2", etc.
  std::set<std::string> outputChannelNames; // Using set instead of vector to
                                            // automatically deduplicate
  std::set<std::string> inputChannelNames;  // Using set instead of vector to
                                            // automatically deduplicate
  ...
  if (!outputChannelNames.empty()) {
    std::cout << "Found " << outputChannelNames.size()
  ...
  if (!inputChannelNames.empty()) {
    std::cout << "Found " << inputChannelNames.size()
  ...
  std::set<std::string> stereoOutputChannels; // Using set instead of vector to
                                              // automatically deduplicate
  std::set<std::string> stereoInputChannels;  // Using set instead of vector to
                                              // automatically deduplicate
  ...
  if (!stereoOutputChannels.empty()) {
    std::cout << "Found " << stereoOutputChannels.size()
  ...
  if (!stereoInputChannels.empty()) {
    std::cout << "Found " << stereoInputChannels.size()
  ...
  std::cout << "  Stereo Pairs: " << totalStereoOutputs << " ("
            << totalStereoOutputs * 2 << " channels)" << std::endl;
  std::cout << "  Total Physical Outputs: " << totalPhysicalOutputs
            << std::endl;

  std::cout << "Input Channels:" << std::endl;
  std::cout << "  Mono: " << totalMonoInputs << std::endl;
  std::cout << "  Stereo Pairs: " << totalStereoInputs << " ("
            << totalStereoInputs * 2 << " channels)" << std::endl;
  std::cout << "  Total Physical Inputs: " << totalPhysicalInputs << std::endl;

  std::cout << "Total I/O Channels: "
            << (totalPhysicalOutputs + totalPhysicalInputs) << std::endl;

  // Validate total channel count
  const int MAX_REASONABLE_TOTAL_CHANNELS =
      128; // Adjust based on device capabilities
  if (totalPhysicalOutputs + totalPhysicalInputs >
      MAX_REASONABLE_TOTAL_CHANNELS) {
    std::cout << "Warning: Total channel count ("
              << (totalPhysicalOutputs + totalPhysicalInputs)
              << ") exceeds reasonable limit of "
              << MAX_REASONABLE_TOTAL_CHANNELS << std::endl;
  }
