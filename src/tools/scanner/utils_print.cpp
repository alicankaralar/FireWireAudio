...
void printDeviceInfo(const FireWireDevice &device) {
  ...
  if (device.diceChipType != DiceChipType::Unknown ||
    ...
    if (device.currentConfig != DiceConfig::Unknown) {
    ...
    if (!device.diceRegisters.empty()) {
      ...
      for (const auto &regPair : device.diceRegisters) {
        ...
        std::cout << "0x" << std::hex << std::setw(16) << std::setfill('0')
                  << addr << std::dec << " | 0x" << std::hex << std::setw(8)
                  << std::setfill('0') << hostValue << std::dec << " | "
