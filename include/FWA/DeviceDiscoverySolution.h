...
// Helper method to determine if a FireWire device is a DICE device by checking
...
inline bool IsDiceDevice(IOFireWireDeviceInterface **deviceInterface) {
  ...
  // Static list of known DICE (VendorID, ModelID) pairs from libffado
  // configuration
  ...
  // Read the vendor ID and model ID using IOFireWireConfigDirectoryInterface
  // methods
  UInt32 vendorId = 0;
  UInt32 modelId = 0;
  ...
  IOReturn status =
      (*configDir)
          ->GetKeyValue_UInt32(configDir, kFWVendorID, &vendorId, nullptr);
  if (status == kIOReturnSuccess) {
    status = (*configDir)
                 ->GetKeyValue_UInt32(configDir, kFWModelID, &modelId, nullptr);
