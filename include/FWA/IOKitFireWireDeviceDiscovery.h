...
public:
   ...
   */
  ~IOKitFireWireDeviceDiscovery() noexcept override;

  /**
   * @brief Start device discovery with callback notifications
   * @param callback Function to be called when devices are
   * connected/disconnected
   * @return Success or error status
   */
  std::expected<void, IOKitError>
  startDiscovery(DeviceNotificationCallback callback) override;
  ...
  getDeviceByGuid(std::uint64_t guid) override;
...
private:
  ...
  std::vector<std::shared_ptr<AudioDevice>>
      devices_;             ///< List of discovered devices
   ...
   */
  std::expected<std::shared_ptr<AudioDevice>, IOKitError>
  createAudioDevice(io_object_t device);
  ...
  std::shared_ptr<DeviceController>
      deviceController_; ///< Device controller for device operations
  std::weak_ptr<DeviceController>
      deviceControllerWk_; ///< Use weak_ptr if passing shared_ptr? Review this
                           ///< later.
