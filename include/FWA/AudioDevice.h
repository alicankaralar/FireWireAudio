 ...
 *
 * AudioDevice class encapsulates a FireWire audio device, providing access to
 * its properties, controls, and interfaces. It manages the device's lifecycle
 * and provides methods to interact with the device's capabilities.
 */
...
public:
     ...
     * @param fwDeviceService IOKit service representing the IOFireWireDevice
     */
    AudioDevice(
        std::uint64_t guid,
        const std::string& deviceName,
        const std::string& vendorName,
        io_service_t fwDeviceService,
     ...
     * @return std::shared_ptr<CommandInterface> Interface for sending commands to
     * the device
     */
    ...
    std::expected<void, IOKitError> disconnectMusicPlug(uint8_t musicPlugType, uint16_t musicPlugID);

    std::expected<void, IOKitError> setUnitIsochPlugStreamFormat(PlugDirection direction, uint8_t plugNum, const AudioStreamFormat& format);
     ...
     * @param musicPlugType The type of the music input plug (e.g., 0x00 Audio,
     * 0x01 MIDI).
     * @param musicPlugID The ID of the music input plug.
     * @param newDestSubunitPlugID The ID of the *new* destination subunit plug to
     * connect to.
    ...
    /**
     * @brief Disconnects all music input plugs from a specific destination
     * subunit plug. (Subfunction 0x03 of DESTINATION PLUG CONFIGURE)
     * @param fromDestSubunitPlugID The ID of the destination subunit plug to
     * disconnect from.
     * @return Success or IOKitError.
     */
...
private:
    ...
    std::vector<uint8_t> buildSetStreamFormatControlCmd(PlugDirection direction, uint8_t plugNum, const std::vector<uint8_t>& formatBytes);

    std::expected<void, IOKitError>
    checkControlResponse(const std::expected<std::vector<uint8_t>, IOKitError>& result, const char *commandName);

    std::expected<void, IOKitError>
    checkDestPlugConfigureControlSubcommandResponse(const std::vector<uint8_t>& response, const char *commandName);
