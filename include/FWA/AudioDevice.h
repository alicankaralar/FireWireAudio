...
public:
   ...
   */
  AudioDevice(std::uint64_t guid, const std::string &deviceName,
              const std::string &vendorName, io_service_t fwDeviceService,
   ...
   */
  IONotificationPortRef getNotificationPort() const {
  ...
  // --- NEW CONTROL METHODS ---
  std::expected<void, IOKitError> connectMusicPlug(uint8_t musicPlugType,
                                                   uint16_t musicPlugID,
                                                   uint8_t destSubunitPlugID,
                                                   uint8_t streamPosition0,
                                                   uint8_t streamPosition1);

  std::expected<void, IOKitError> disconnectMusicPlug(uint8_t musicPlugType,
                                                      uint16_t musicPlugID);

  std::expected<void, IOKitError>
  setUnitIsochPlugStreamFormat(PlugDirection direction, uint8_t plugNum,
   ...
   */
  std::expected<void, IOKitError>
  disconnectAllMusicPlugs(uint8_t fromDestSubunitPlugID);
...
private:
  ...
  std::vector<uint8_t>
  buildSetStreamFormatControlCmd(PlugDirection direction, uint8_t plugNum,
