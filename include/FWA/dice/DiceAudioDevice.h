...
public:
   ...
   */
  DiceAudioDevice(std::uint64_t guid, const std::string &deviceName,
                  const std::string &vendorName, io_service_t avcUnit,
   ...
   * @return Pointer to the EAP interface or nullptr if not supported by the
   * device
   */
  DiceEAP *getEAP() const { return m_supportsEAP ? eap_.get() : nullptr; }
...
private:
  ...
  std::expected<std::vector<uint32_t>, IOKitError> readRegBlock(uint64_t offset,
                                                                size_t length);
  std::expected<void, IOKitError>
  writeRegBlock(uint64_t offset, const uint32_t *data, size_t length);

  // EAP section-aware access methods
  std::expected<uint32_t, IOKitError> readEAPReg(DICE::EAPSection section,
                                                 uint32_t offset);
  std::expected<void, IOKitError> writeEAPReg(DICE::EAPSection section,
                                              uint32_t offset, uint32_t data);
  std::expected<std::vector<uint32_t>, IOKitError>
  readEAPBlock(DICE::EAPSection section, uint32_t offset, size_t length);
  std::expected<void, IOKitError> writeEAPBlock(DICE::EAPSection section,
                                                uint32_t offset,
                                                const uint32_t *data,
                                                size_t length);

  // EAP helper methods
  uint64_t eapOffsetGen(DICE::EAPSection section, uint32_t offset,
                        size_t length);
  const DICE::EAPSectionInfo *getEAPSectionInfo(DICE::EAPSection section) const;
  bool isEAPSectionReadOnly(DICE::EAPSection section) const;

  std::expected<uint32_t, IOKitError> readGlobalReg(uint32_t offset);
  std::expected<void, IOKitError> writeGlobalReg(uint32_t offset,
                                                 uint32_t data);
  std::expected<std::vector<uint32_t>, IOKitError>
  readGlobalRegBlock(uint32_t offset, size_t length);
  std::expected<void, IOKitError>
  writeGlobalRegBlock(uint32_t offset, const uint32_t *data, size_t length);
  uint64_t globalOffsetGen(uint32_t offset, size_t length);

  std::expected<uint32_t, IOKitError> readTxReg(unsigned int i,
                                                uint32_t offset);
  std::expected<void, IOKitError> writeTxReg(unsigned int i, uint32_t offset,
                                             uint32_t data);
  std::expected<std::vector<uint32_t>, IOKitError>
  readTxRegBlock(unsigned int i, uint32_t offset, size_t length);
  std::expected<void, IOKitError> writeTxRegBlock(unsigned int i,
                                                  uint32_t offset,
                                                  const uint32_t *data,
                                                  size_t length);
  uint64_t txOffsetGen(unsigned int i, uint32_t offset, size_t length);

  std::expected<uint32_t, IOKitError> readRxReg(unsigned int i,
                                                uint32_t offset);
  std::expected<void, IOKitError> writeRxReg(unsigned int i, uint32_t offset,
                                             uint32_t data);
  std::expected<std::vector<uint32_t>, IOKitError>
  readRxRegBlock(unsigned int i, uint32_t offset, size_t length);
  std::expected<void, IOKitError> writeRxRegBlock(unsigned int i,
                                                  uint32_t offset,
                                                  const uint32_t *data,
                                                  size_t length);
  ...
  std::expected<bool, IOKitError> maskedCheckZeroGlobalReg(uint32_t offset,
                                                           uint32_t mask);
  std::expected<bool, IOKitError> maskedCheckNotZeroGlobalReg(uint32_t offset,
                                                              uint32_t mask);
  ...
  // Flag indicating if the device supports EAP
  bool m_supportsEAP = true; // Assume support by default
};
