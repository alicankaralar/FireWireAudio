...
private:
    ...
    std::expected<uint32_t, IOKitError> readReg(uint64_t offset, uint64_t base = DICE_REGISTER_BASE);
    ...
    std::expected<uint32_t, IOKitError> readEAPReg(DICE::EAPSection section, uint32_t offset);
    std::expected<void, IOKitError> writeEAPReg(DICE::EAPSection section, uint32_t offset, uint32_t data);
    std::expected<std::vector<uint32_t>, IOKitError> readEAPBlock(DICE::EAPSection section, uint32_t offset, size_t length);
    std::expected<void, IOKitError> writeEAPBlock(DICE::EAPSection section, uint32_t offset, const uint32_t *data, size_t length);

    // EAP helper methods
    uint64_t eapOffsetGen(DICE::EAPSection section, uint32_t offset, size_t length);
