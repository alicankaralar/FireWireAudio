 ...
 *
 * This class provides a user-friendly interface for configuring routing on DICE
 * devices. It abstracts the low-level details and provides a matrix-style
...
public:
   ...
   * @return Success or error status
   */
  std::expected<void, IOKitError> connect(uint8_t sourceId,
                                          uint8_t sourceChannel, uint8_t destId,
                                          uint8_t destChannel);
   ...
   * @return Success or error status
   */
  std::expected<void, IOKitError> connect(const RoutePoint &source,
   ...
   * @return Success or error status
   */
  std::expected<void, IOKitError> disconnect(uint8_t destId,
                                             uint8_t destChannel);
   ...
   */
  std::expected<RoutePoint, IOKitError> getSource(uint8_t destId,
   ...
   * @return Source point or error
   */
  std::expected<RoutePoint, IOKitError>
  getSource(const RoutePoint &destination) const;
   ...
   */
  std::expected<std::vector<RoutePoint>, IOKitError>
  getDestinations(uint8_t sourceId, uint8_t sourceChannel) const;
   ...
   */
  std::expected<std::vector<RoutePoint>, IOKitError>
  getDestinations(const RoutePoint &source) const;
   ...
   *
   * Returns a matrix with port names and connection status, suitable for UI
   * display. The matrix includes row headers (source names) and column headers
   * (destination names).
   *
   ...
   *
   * @return String containing a formatted text representation of the routing
   * matrix
   */
   ...
   * @return Success or error status
   */
  std::expected<void, IOKitError> setMidiInput(unsigned int port,
                                               const RoutePoint &dest);
   ...
   * @return Success or error status
   */
  std::expected<void, IOKitError> setMidiOutput(unsigned int port,
   ...
   * @return Success or error status
   */
  std::expected<void, IOKitError> applyChannelFormat(int txChannels,
                                                     int rxChannels);
