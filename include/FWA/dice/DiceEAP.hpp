...
public:
   ...
   * @return Success or error status
   */
  std::expected<void, IOKitError> createRoute(unsigned char srcId,
                                              unsigned char dstId);
   ...
   * @return Success or error status
   */
  std::expected<void, IOKitError> setupRoute(unsigned char srcId,
                                             unsigned char dstId);
  ...
  getSourceForDestination(unsigned char dstId);
  ...
  getDestinationsForSource(unsigned char srcId);
...
public:
   ...
   */
  enum class CommandStatus { Error, Timeout, Busy, Done };
...
public:
   ...
   */
  DiceStandaloneConfig *getStandaloneConfig() {
    return standaloneConfig_.get();
  }
   ...
   * @return Success or error status
   */
  std::expected<void, IOKitError> readReg(RegBase base, uint32_t offset,
                                          uint32_t *value);
   ...
   * @return Success or error status
   */
  std::expected<void, IOKitError> writeReg(RegBase base, uint32_t offset,
                                           uint32_t value);
   ...
   * @return Success or error status
   */
  std::expected<void, IOKitError> readRegBlock(RegBase base, uint32_t offset,
   ...
   * @return Success or error status
   */
  std::expected<void, IOKitError> writeRegBlock(RegBase base, uint32_t offset,
                                                const uint32_t *data,
                                                size_t size);
   ...
   * @return Human-readable name string
   */
  std::string getDestinationName(DICE::RouteDestination dstId,
  ...
  readApplicationSpace(uint32_t offset, size_t size);
...
private:
   ...
   * @return Success or error status
   */
  std::expected<void, IOKitError> addRoute(DICE::RouteSource srcId,
